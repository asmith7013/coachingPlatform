// Script to update Grade 7 lessons in scope-and-sequence collection with standards from standards7.json
// Run with: mongosh "$DATABASE_URL" --file /path/to/this/script.js

// Load the standards data
const fs = require('fs');
const standardsData = JSON.parse(fs.readFileSync('/Users/alexsmith/ai-coaching-platform/docs/standards7.json', 'utf8'));
...
print('Starting to update Grade 7 standards in scope-and-sequence collection...\n');

// Process each lesson from standards7.json
standardsData.forEach(lesson => {
  totalProcessed++;

  // Parse unit number from "7.1" format -> 1
  const unitNumber = parseInt(lesson.unit.split('.')[1]);
  const grade = lesson.grade.toString(); // Convert 7 to "7"
