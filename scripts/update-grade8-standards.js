// Script to update Grade 8 lessons in scope-and-sequence collection with standards from standards8.json
...
print('Starting to update Grade 8 standards in scope-and-sequence collection...\n');

// Process each lesson from standards8.json
standardsData.forEach(lesson => {
  totalProcessed++;

  // Parse unit number from "8.1" format -> 1
  const unitNumber = parseInt(lesson.unit.split('.')[1]);
  const grade = lesson.grade.toString(); // Convert 8 to "8"
