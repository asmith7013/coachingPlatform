        ...
        {selectedSection && selectedUnit !== null && selectedLessonSection ? (
          lessons.length === 0 ? (
            ...
            <div className="space-y-8">
              {/* Group assignments by type: lessons first, then mastery-checks */}
              {(() => {
                ...
                return (
                  <>
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
                            {lessonAssignments.length} {lessonAssignments.length === 1 ? 'Lesson' : 'Lessons'}
                          </span>
                        </div>
                        <div className="space-y-6">
                          {lessonAssignments.map((assignment) => (
                            <AssignmentCard
                              key={`${assignment.section}-${assignment.unitLessonId}-${assignment.assignmentType}`}
                              assignment={assignment}
                              progressData={progressData}
                              syncing={syncing === `${assignment.unitLessonId}-${assignment.assignmentType || 'default'}`}
                    ...
                    {/* Mastery Checks Section */}
                    {masteryCheckAssignments.length > 0 && (
                      <div>
                        <div className="flex items-center gap-3 mb-4">
                          <h2 className="text-2xl font-bold text-gray-900">Mastery Checks</h2>
                          <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">
                            {masteryCheckAssignments.length} {masteryCheckAssignments.length === 1 ? 'Check' : 'Checks'}
                          </span>
                        </div>
                        <div className="space-y-6">
                          {masteryCheckAssignments.map((assignment) => (
                            <AssignmentCard
                              key={`${assignment.section}-${assignment.unitLessonId}-${assignment.assignmentType}`}
                              assignment={assignment}
                              progressData={progressData}
