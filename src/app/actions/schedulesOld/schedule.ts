...
import {
  ...
  BellScheduleModel,
} from "@/lib/schema/mongoose-schema/schedules/schedule.model";
import {
...
export async function fetchBellSchedules(params: QueryParams) {
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "DEPRECATED: fetchBellSchedules from schedulesOld is deprecated. Use @/app/actions/schedules/bell-schedule instead.",
    );
  }
...
export async function createBellSchedule(data: BellScheduleInput) {
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "DEPRECATED: createBellSchedule from schedulesOld is deprecated. Use @/app/actions/schedules/bell-schedule instead.",
    );
  }
...
export async function updateBellSchedule(
  ...
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "DEPRECATED: updateBellSchedule from schedulesOld is deprecated. Use @/app/actions/schedules/bell-schedule instead.",
    );
  }
...
export async function deleteBellSchedule(id: string) {
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "DEPRECATED: deleteBellSchedule from schedulesOld is deprecated. Use @/app/actions/schedules/bell-schedule instead.",
    );
  }
...
export async function fetchBellScheduleById(id: string) {
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "DEPRECATED: fetchBellScheduleById from schedulesOld is deprecated. Use @/app/actions/schedules/bell-schedule instead.",
    );
  }
...
export async function getActiveCycleDayForDate(schoolId: string, date: string) {
  return withDbConnection(async () => {
    try {
      ...
      const bellSchedules = await BellScheduleModel.find({
        school: schoolId,
      }).exec();
      ...
      for (const schedule of bellSchedules) {
        const assignedDay = schedule.assignedCycleDays?.find(
          (day: { date: string; blockDayType: string }) => day.date === date,
        );
...
interface TeacherScheduleWithEmail
  extends Omit<TeacherScheduleInput, "teacher"> {
  teacherEmail: string;
  teacherName: string;
...
async function findStaffMemberByEmail(
  ...
  try {
    ...
    const allStaff = await NYCPSStaffModel.find({})
      .select("email staffName")
      .exec();
...
export async function createMasterSchedule(
  ...
  return withDbConnection(async () => {
    ...
    try {
      ...
      console.log(
        `📊 Database connection verified. Total staff in database: ${staffCount}`,
      );
      ...
      for (let i = 0; i < schedules.length; i++) {
        ...
        try {
          ...
          const staffMember = await findStaffMemberByEmail(
          ...
          console.log(
            `✅ Found staff member: ${staffMember.staffName} (ID: ${staffMember._id})`,
          );
          console.log(
            `📧 Matched email: "${staffMember.email}" with input: "${schedule.teacherEmail}"`,
          );
          ...
          const scheduleData: TeacherScheduleInput = {
            ...
            assignments:
              (schedule.assignments as TeacherScheduleReference[]) || [],
          ...
          console.log(`📝 Creating schedule data:`, {
            ...
            assignmentsCount: (
              scheduleData.assignments as TeacherScheduleReference[]
            ).length,
          });
