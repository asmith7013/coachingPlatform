...
export async function fetchCapWeeklyPlans(params: QueryParams = DEFAULT_QUERY_PARAMS) {
  return withDbConnection(() => capWeeklyPlanActions.fetch(params));
}

export async function fetchCapWeeklyPlansByCapId(capId: string) {
  return withDbConnection(() => capWeeklyPlanActions.fetch({ 
...
export async function updateCapWeeklyPlan(id: string, data: Partial<CapWeeklyPlanInput>) {
