...
import {
  parseSchoolSlug,
  createSchoolSlug,
} from "@data-processing/transformers/utils/school-slug-utils";
 ...
 */
export async function getSchoolIdFromSlug(
  slug: string,
): Promise<string | null> {
  "use server";
  ...
  return withDbConnection(async () => {
    try {
      ...
      const schools = await SchoolModel.find(
        {},
        { district: 1, schoolNumber: 1 },
      );

      for (const school of schools) {
        const schoolSlug = createSchoolSlug(
          school.district,
          school.schoolNumber,
        );
        if (schoolSlug === slug) {
