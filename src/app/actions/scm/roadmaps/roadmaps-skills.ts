  ...
  return withDbConnection(async () => {
    try {
      ...
      if (existingSkill) {
        ...
        const units = existingSkill.get("units") as unknown as Array<{
          grade: string;
          unitTitle: string;
          unitNumber: number;
        }>;
        const unitExists = units.some(
          (u) =>
            u.grade === skillData.unit.grade &&
            u.unitTitle === skillData.unit.unitTitle,
        );
 ...
 */
export async function updateRoadmapsSkillContent(
  skillData: z.infer<typeof RoadmapsSkillInputZodSchema>,
) {
  return withDbConnection(async () => {
    try {
      console.log(
        `🔍 [DB] updateRoadmapsSkillContent called for skill ${skillData.skillNumber}`,
      );
      const practiceProblemsInput = skillData.practiceProblems as
        | Array<{
        ...
        | undefined;
      console.log(
        `🔍 [DB] Input has ${practiceProblemsInput?.length || 0} practice problems`,
      );

      // Validate input
      const validatedData = RoadmapsSkillInputZodSchema.parse(skillData);
      const practiceProblemsValidated = validatedData.practiceProblems as
        | Array<{
            problemNumber: number;
            screenshotUrl: string;
            scrapedAt: string;
        ...
        | undefined;
      console.log(
        `🔍 [DB] After validation: ${practiceProblemsValidated?.length || 0} practice problems`,
      );
      ...
      if (existingSkill) {
        ...
        const updatedPracticeProblems = updatedSkill?.get(
          "practiceProblems",
        ) as
          | Array<{
              problemNumber: number;
          ...
          | undefined;
        console.log(
          `🔍 [DB] Updated skill now has ${updatedPracticeProblems?.length || 0} practice problems`,
        );
        ...
        console.log(
          `✨ Creating new skill ${validatedData.skillNumber}:`,
          validatedData.title,
        );
...
export async function addImLessonToSkill(data: {
  ...
  return withDbConnection(async () => {
    try {
      ...
      const imLessons = existingSkill.get("imLessons") as unknown as
        | Array<{
            unitNumber: number;
            lessonNumber: number;
            lessonName?: string;
      ...
      if (
        ...
        if (
          ...
          data.grade &&
          existingLesson.grade === data.grade
        ) {
          return {
            success: true,
          ...
          await RoadmapsSkillModel.findOneAndUpdate(
            ...
            },
            {
              $set: {
                ...
                "imLessons.$.lessonName":
                  data.lessonName || existingLesson.lessonName,
                updatedAt: new Date().toISOString(),
              },
            },
            { new: true },
    ...
    try {
      ...
      for (const skillData of skillsData) {
        try {
          ...
          if (existingSkill) {
            console.log(
              `⚠️ Updating existing skill ${validatedData.skillNumber}:`,
              validatedData.title,
            );
