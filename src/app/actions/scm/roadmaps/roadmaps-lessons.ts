  ...
  return withDbConnection(async () => {
    try {
      ...
      if (validatedData.skillNumber) {
        ...
        if (existingLesson) {
          console.log(
            `⚠️ Skill ${validatedData.skillNumber} already exists, updating instead`,
          );
          return updateRoadmapsLesson(
...
export async function bulkCreateRoadmapsLessons(lessonsData: unknown) {
  return withDbConnection(async () => {
    try {
      ...
      const lessonsArray = z
        .array(RoadmapsLessonInputZodSchema)
        .parse(lessonsData);
      ...
      for (const lessonData of lessonsArray) {
        try {
          ...
          const existingLesson = lessonData.skillNumber
            ? await RoadmapsLessonModel.findOne({
          ...
          if (existingLesson) {
            console.log(
              `⚠️ Updating existing skill ${lessonData.skillNumber}:`,
              lessonData.title,
            );
...
export async function updateRoadmapsLesson(
  lessonId: string,
  updateData: unknown,
) {
  return withDbConnection(async () => {
    try {
      ...
      const validatedData =
        RoadmapsLessonInputZodSchema.partial().parse(updateData);
...
export async function getRoadmapsLessons(
  options: {
    search?: string;
    successOnly?: boolean;
    limit?: number;
  ...
  return withDbConnection(async () => {
    try {
      const { search, successOnly = true, limit = 100 } = options;
      ...
      const lessons = await RoadmapsLessonModel.find(query)
        .sort({ scrapedAt: -1 })
        .limit(limit)
        .exec();
...
export async function deleteRoadmapsLesson(lessonId: string) {
  return withDbConnection(async () => {
    try {
      ...
      const deletedLesson =
        await RoadmapsLessonModel.findByIdAndDelete(lessonId);
