...
export async function createIMLesson(lessonData: unknown) {
  return withDbConnection(async () => {
    try {
      ...
      if (existingLesson) {
        console.log(
          "⚠️ Lesson with same identifiers already exists, updating instead",
        );
...
export async function fetchIMLessons(
  params: {
    grade?: string;
    unit?: string;
    section?: string;
  ...
  return withDbConnection(async () => {
    try {
      const { grade, unit, section, page = 1, limit = 50, search } = params;
      ...
      const lessons = await RoadmapsLessonModel.find(filter)
