...
const scopeSequenceUnitOrderCrud = createCrudActions({
  model: ScopeSequenceUnitOrderModel as unknown as Parameters<typeof createCrudActions>[0]['model'],
...
export const fetchScopeSequenceUnitOrderById = scopeSequenceUnitOrderCrud.fetchById;
 ...
 */
export async function fetchUnitOrderByScopeTag(scopeSequenceTag: string): Promise<{
  success: boolean;
  data?: UnitOrderItem[];
  error?: string;
}> {
  return withDbConnection(async () => {
    try {
      const doc = await ScopeSequenceUnitOrderModel.findOne({ scopeSequenceTag }).lean<UnitOrderDoc>();
