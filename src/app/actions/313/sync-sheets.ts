  ...
  return withDbConnection(async () => {
    try {
        ...
        try {
          const validatedCompletions = result.data.lessonCompletions.map(completion => {
            if (completion.completionType === 'zearn') {
              ...
              return SnorklCompletionInputZodSchema.parse(completion);
            }
          });
      ...
      return {
        ...
        processingErrors: result.errors.length > 0 ? result.errors : undefined,
...
export async function triggerManualSync() {
  ...
  return syncSheetsData(spreadsheetId);
}
