...
export async function fetchStudentData(studentId: string) {
  return withDbConnection(async () => {
    try {
      console.log(`🔍 Server action: Fetching student data for ID: ${studentId}`);
      
      const result = await StudentDashboardService.fetchStudentDataFromSheets(studentId);
...
export async function authenticateStudent(email: string, studentId: string) {
  return withDbConnection(async () => {
    try {
      console.log(`🔐 Server action: Authenticating student ${studentId} with email: ${email}`);
      ...
      const result = await StudentDashboardService.validateStudentEmail(email.trim(), studentId.trim());
