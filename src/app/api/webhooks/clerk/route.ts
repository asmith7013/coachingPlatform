...
import {
  handleUserCreation,
  handleUserSync,
  handleOrganizationSync,
  handleUserDeletion,
  OrganizationWebhookData,
  UserWebhookData,
} from "@server/api/handlers/clerk-webhook";
  ...
  try {
    ...
    switch (type) {
      ...
      case "organization.updated":
        const orgResult = await handleOrganizationSync(
          data as OrganizationWebhookData,
        );
        return NextResponse.json(orgResult);

      case "user.deleted":
        const deleteResult = await handleUserDeletion(
          data as unknown as UserWebhookData,
        );
