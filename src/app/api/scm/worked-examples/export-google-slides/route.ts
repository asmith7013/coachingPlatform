...
export async function POST(request: NextRequest) {
  console.log(
    "[export-google-slides] ========== ROUTE HANDLER STARTED ==========",
  );
  ...
  try {
    ...
    if (slug) {
      console.log(
        `[export-google-slides] Slug: "${slug}" (will save URL to database)`,
      );
    }
    ...
    const result = await generatePptxFromSlides(slides, {
      ...
      onProgress: (current, total, message, step) => {
        console.log(
          `[export-google-slides] [${step}] ${message} (${current}/${total})`,
        );
      },
    });
    ...
    console.log(
      `[export-google-slides] Base64 size: ${(result.pptxBase64.length / 1024).toFixed(1)} KB`,
    );

    // Validate PPTX was generated
    if (!result.pptxBase64 || result.pptxBase64.length === 0) {
      console.error(
        "[export-google-slides] PPTX generation returned empty content",
      );
      return NextResponse.json(
        { error: "PPTX generation failed - empty content" },
        { status: 500 },
      );
    }
    ...
    console.log(
      `[export-google-slides] Buffer size: ${(pptxBuffer.length / 1024).toFixed(1)} KB`,
    );

    // Validate buffer is not empty
    if (pptxBuffer.length === 0) {
      console.error(
        "[export-google-slides] PPTX buffer is empty after base64 decode",
      );
      return NextResponse.json(
        { error: "PPTX generation failed - invalid content" },
        { status: 500 },
      );
    }
    ...
    if (pkSignature !== "PK") {
      console.error(
        `[export-google-slides] Invalid PPTX signature: ${pkSignature} (expected PK)`,
      );
      return NextResponse.json(
        { error: "PPTX generation failed - invalid file format" },
        { status: 500 },
      );
    }
    ...
    const uploadResult = await uploadPptxToUserGoogleSlides(
      pptxBuffer,
      result.displayTitle,
    );
    ...
    if (!uploadResult.success) {
      console.error(
        "[export-google-slides] Upload failed:",
        uploadResult.error,
      );
    ...
    if (slug && uploadResult.url) {
      try {
        await withDbConnection(async () => {
          ...
          if (updateResult) {
            console.log(
              `[export-google-slides] Saved URL to database for slug: ${slug}`,
            );
          } else {
            console.log(
              `[export-google-slides] No deck found with slug: ${slug} (URL not saved)`,
            );
          }
        });
      } catch (dbError) {
        ...
        console.error(
          "[export-google-slides] Failed to save URL to database:",
          dbError,
        );
      }
    }
    ...
    console.error(
      "[export-google-slides] Error type:",
      error?.constructor?.name,
    );
    console.error(
      "[export-google-slides] Error message:",
    ...
    if (error instanceof RenderError) {
      console.error(
        "[export-google-slides] Returning RenderError:",
        error.message,
      );
    ...
    if (
      ...
      const puppeteerError = handlePuppeteerError(
        error,
        "Google Slides export",
      );
      console.error(
        "[export-google-slides] Returning puppeteer error:",
        puppeteerError,
      );
      return NextResponse.json({ error: puppeteerError }, { status: 500 });
    }
    ...
    console.error(
      "[export-google-slides] Returning generic error:",
      finalError,
    );
