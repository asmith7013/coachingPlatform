 ...
 */
export function validateStackedElements(
  html: string,
  slideIndex: number = 0,
): SlideValidation {
  const elements = extractElementBounds(html);
  ...
  const leftCol = elements.filter(
    (e) =>
      e.x < 400 &&
      e.regionType !== "badge" &&
      e.regionType !== "title" &&
      e.regionType !== "subtitle" &&
      e.regionType !== "footnote",
  );
  const rightCol = elements.filter(
    (e) =>
      e.x >= 400 && e.regionType !== "cfu-box" && e.regionType !== "answer-box",
  );
