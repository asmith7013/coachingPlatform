...
export function extractBgColor(style: string | undefined): string | undefined {
  ...
  const rgbaMatch = style.match(
    /background:\s*rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i,
  );
  if (rgbaMatch) {
 ...
 */
export function extractFontSize(
...
export function extractFontFamily(
  ...
  if (match) {
    ...
    if (
      firstFont.toLowerCase().includes("serif") &&
      !firstFont.toLowerCase().includes("sans")
    )
      return "Georgia";
