...
const scoreCardVariants = tv({
  slots: {
    container: "flex items-center justify-between p-4 rounded-lg border border-gray-200 min-h-[80px]",
    label: "text-lg font-medium text-gray-900",
    scoreContainer: "flex items-center justify-center w-16 h-16 rounded-lg text-white font-bold text-xl",
...
export function IPGScoreCard({ label, score, yesNo }: IPGScoreCardProps) {
  const { container, label: labelClass, scoreContainer, yesNoContainer } = scoreCardVariants();
  ...
  return (
    <div className={container()}>
      ...
      {score !== undefined ? (
        <div className={scoreContainer({ score: score ? (score as 1 | 2 | 3 | 4) : "null" })}>
          {score || 'N/A'}
        </div>
      ) : (
        <div className={yesNoContainer({ yesNo: yesNo ? (yesNo as "Yes" | "No") : "null" })}>
          {yesNo || 'N/A'}
        </div>
      )}
    </div>
