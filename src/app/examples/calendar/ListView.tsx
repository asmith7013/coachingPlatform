  ...
  const getCycleInfo = (visit: Visit): { name: string; className: string } => {
    ...
    if (visit.sessionName.toLowerCase().includes('initial walkthrough')) {
      return { 
        ...
        className: 'text-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800/30' 
      };
    }
    ...
    for (const cycleDate of cycles['Winter Cycle']) {
      if (cycleDate.date === visitDate) {
        return { 
          name: 'Fall Cycle', 
          className: 'text-amber-700 bg-amber-50 dark:text-amber-400 dark:bg-amber-950/30' 
        };
      }
    }
    ...
    for (const cycleDate of cycles['Spring Cycle']) {
      if (cycleDate.date === visitDate) {
        return { 
          name: 'Spring Cycle', 
          className: 'text-emerald-700 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-950/30' 
        };
      }
    }
          ...
          {sortedVisits.map((visit) => {
            ...
            return (
              <tr key={visit.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  <span className={`inline-flex text-xs font-medium px-3 py-2 rounded-full ${cycleInfo.className}`}>
                    {cycleInfo.name}
                  </span>
                </td>
              </tr>
