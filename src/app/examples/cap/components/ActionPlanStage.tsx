...
export const ActionPlanStage: React.FC<ActionPlanStageProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md mb-8 overflow-hidden ${className}`}
    >
      {/* Left sidebar with stage number and title - mirrors PDF layout */}
      <div className="flex flex-col md:flex-row">
        <div className={`${bgColorClass} text-white p-4 md:w-1/5`}>
          <div className="font-bold">Stage {number}:</div>
          <div className="text-lg font-semibold">{title}</div>
        </div>

        {/* Main content area */}
        <div className="p-6 md:w-4/5">{children}</div>
      </div>
    </div>
  );
};
