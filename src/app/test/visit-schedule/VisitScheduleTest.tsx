...
export default function VisitScheduleTest() {
  const [visitSchedule, setVisitSchedule] = useState<VisitSchedule>(
  ...
  const getExistingBlock = useCallback(
    (teacherId: string, periodNumber: number) => {
      return visitSchedule.timeBlocks.find(
        (block) =>
          block.teacherId === teacherId && block.periodNumber === periodNumber,
      );
    },
  ...
  const updateVisitBlock = useCallback(
    (
      ...
      setLastAction(
        `Added ${assignment.replace("_", " ")} visit for ${teacher.staffName} during Period ${periodNumber}`,
      );
    },
    [],
  );
  ...
  const removeVisitBlock = useCallback(
    (teacherId: string, periodNumber: number) => {
      const teacher = TEST_TEACHERS.find((t) => t._id === teacherId);
      setVisitSchedule((prev) => ({
        ...prev,
      ...
      setLastAction(
        `Removed visit for ${teacher?.staffName} during Period ${periodNumber}`,
      );
    },
    [],
  );
  ...
  const summary = useMemo(() => {
    ...
    const teachersScheduled = new Set(
      visitSchedule.timeBlocks.map((b) => b.teacherId),
    ).size;
    const fullPeriods = visitSchedule.timeBlocks.filter(
      (b) => b.assignment === ScheduleAssignment.FULL_PERIOD,
    ).length;
    const halfPeriods = visitSchedule.timeBlocks.filter(
      (b) => b.assignment !== ScheduleAssignment.FULL_PERIOD,
    ).length;
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          City Island School - Interactive Visit Scheduling
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {TEST_TEACHERS.map((teacher) => (
              <Button
                key={teacher._id}
                intent={
                  selectedTeacher === teacher._id ? "primary" : "secondary"
                }
                appearance={
                  selectedTeacher === teacher._id ? "solid" : "outline"
                }
                onClick={() =>
                  setSelectedTeacher(
        ...
        <Card className="mb-6">
          <Card.Header>
            <h2 className="text-xl font-semibold">
              Schedule Visits for{" "}
              {TEST_TEACHERS.find((t) => t._id === selectedTeacher)?.staffName}
            </h2>
          </Card.Header>
          <Card.Body>
            <div className="grid gap-4">
              {BELL_SCHEDULE.map((period) => {
                const existingBlock = getExistingBlock(
                  selectedTeacher,
                  period.periodNumber,
                );
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        className={getAssignmentButtonStyle(
                          selectedTeacher,
                          period.periodNumber,
          ...
          {visitSchedule.timeBlocks.length === 0 ? (
            ...
            <div className="space-y-2">
              {visitSchedule.timeBlocks
                ...
                .map((block, index) => {
                  const teacher = TEST_TEACHERS.find(
                    (t) => t._id === block.teacherId,
                  );
                  const period = BELL_SCHEDULE.find(
                    (p) => p.periodNumber === block.periodNumber,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <Button
                        ...
                        textSize="sm"
                        onClick={() =>
                          removeVisitBlock(block.teacherId, block.periodNumber)
                        }
                        className="text-red-600 hover:bg-red-50"
                      >
                        Remove
