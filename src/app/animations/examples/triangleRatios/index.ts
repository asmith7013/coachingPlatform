...
export const GEOMETRY_EXAMPLES: ExampleSketch[] = [
  {
    id: "similar-triangles-scale-factor",
    name: "Similar Triangles: Scale Factor",
    description: "Comparing triangles with different scale factors",
    code: `// Similar Triangles: Scale Factor Visualization
...
function drawTriangle(tri) {
  ...
  // Calculate height using Heron's formula for right triangle
...
function showRatio(tri) {
  // Show the ratio calculation
  fill(230, 57, 70);
  textAlign(CENTER, CENTER);
  textSize(20);
  textStyle(BOLD);

  let ratio = tri.long / tri.short;

  text('long side ÷ short side', 200, 320);
  text(tri.long + ' ÷ ' + tri.short + ' = ' + ratio.toFixed(2), 200, 345);
}
...
function keyPressed() {
  ...
  {
    id: "similar-triangles-long-medium",
    name: "Similar Triangles: Long ÷ Medium",
    description: "Comparing long to medium side ratios",
    code: `// Similar Triangles: Long ÷ Medium Side Ratio
let currentTriangle = 0;
let timer = 0;
let displayDelay = 180;  // 3 seconds per triangle
...
function showRatio(tri) {
  ...
  text('medium side ÷ short side', 200, 320);
  text(tri.medium + ' ÷ ' + tri.short + ' = ' + ratio.toFixed(2), 200, 345);
}
