...
import { IMScrapingResponse, CooldownParser, IMLesson } from "../lib/types";
import {
  ChevronDownIcon,
  ChevronUpIcon,
        ...
        <div className="flex items-center p-4 border-b border-gray-200">
          ...
          <div className="ml-4">
            ...
            <p className="text-sm text-gray-500">
              Please wait while we extract content from IM lessons...
            </p>
          </div>
        </div>
        <div className="p-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <div className="space-y-6">
            {claudeExportLessons.map((lesson) => {
              const lessonId = `${lesson.grade}-${lesson.unit}-${lesson.section || "N/A"}-${lesson.lesson}`;
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() =>
                        claudeData &&
                        copyToClipboard(claudeData.formattedForClaude, lessonId)
                      }
                    ...
                    <div className="space-y-4">
                      ...
                      {claudeData.screenshotReferences.length > 0 && (
                        <div>
                          <h5 className="text-sm font-medium text-gray-700 mb-3">
                            Screenshots (
                            {claudeData.screenshotReferences.length})
                          </h5>
                          <ScreenshotDisplay
                      ...
                      <div>
                        <p className="text-sm text-gray-600 mb-2">
                          ...
                          {claudeData.screenshotReferences.length} screenshots
                          included
                        </p>
...
function LessonResultCard({
  ...
  const formatLessonTitle = (lesson: IMLesson) => {
    return `Grade ${lesson.grade} - Unit ${lesson.unit} - Section ${lesson.section?.toUpperCase() || "N/A"} - Lesson ${lesson.lesson}`;
  };
    ...
    <div>
      <SimpleCard
        ...
        subtitle={
          lesson.success
            ? lesson.cooldown
              ? `Content extracted: ${lesson.cooldown.title}`
              : "No cooldown content found"
            : `Error: ${lesson.error}`
        }
      ...
      {isExpanded && (
        <div className="bg-white border border-gray-200 rounded-b-lg shadow-sm -mt-1 px-6 pb-6 space-y-4">
          ...
          <div className="text-sm text-gray-600">
            ...
            {new Date(lesson.scrapedAt).toLocaleString()}
          </div>

          {lesson.cooldown && <CooldownContent cooldown={lesson.cooldown} />}
  ...
  return (
    <div>
      <span className="font-medium text-orange-700">
        Visual Content Screenshots ({screenshots.length} items):
      </span>
        ...
        {screenshots.map((screenshot, index) => (
          ...
          >
            ...
            <div className="p-2 bg-gray-50 text-xs text-gray-600 border-t">
              <div className="truncate" title={screenshot}>
                {screenshot}
              </div>
            </div>
          </div>
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 space-y-3">
      <h4 className="font-semibold text-gray-900">
        Cooldown Content (Summer Project Format)
      </h4>
      ...
      {cooldown.detectedMath.length > 0 && (
        <div>
          <span className="font-medium text-red-700">
            Detected Math Content ({cooldown.detectedMath.length} items):
          </span>
          <div className="ml-2 mt-1 space-y-2">
            {cooldown.detectedMath.map((math, i) => (
              <div key={i} className="bg-red-50 p-2 rounded text-sm">
                <div>
                  <strong>Section:</strong> {math.section}
                </div>
                <div>
                  <strong>Placeholder:</strong> {math.placeholder}
                </div>
                {math.screenreaderText && (
                  <div>
                    <strong>Screen Reader:</strong> {math.screenreaderText}
                  </div>
                )}
      ...
      <div className="border-t pt-2 space-y-1">
        <div className="text-sm text-gray-500">
          <strong>Has Math Content:</strong>{" "}
          {cooldown.hasMathContent ? "Yes" : "No"}
        </div>
        <div className="text-sm text-gray-500">
          <strong>Requires Manual Review:</strong>{" "}
          {cooldown.requiresManualReview ? "Yes" : "No"}
        </div>
