...
import { IMScraper, generateIMUrls as generateUrlsUtil } from '../lib/im-scraper';
...
export async function scrapeIMCooldowns(request: unknown) {
  ...
  try {
    ...
    const { credentials, lessonUrls, delayBetweenRequests, enableClaudeExport } = validatedRequest;
    ...
    const lessons = await scraper.scrapeLessons(lessonUrls, delayBetweenRequests, false, enableClaudeExport);
...
export async function generateIMUrls(params: unknown) {
  try {
    ...
    const { grade, startUnit, endUnit, sectionLessons, delayBetweenRequests } = validated;
...
export async function validateIMCredentials(credentials: unknown) {
  ...
  try {
    ...
    const testUrl = 'https://accessim.org/6-8/grade-6/unit-1/section-a/lesson-1?a=teacher';
    ...
    const isValid = testResult.success && !testResult.error?.includes('authenticate');
...
export async function scrapeIMCooldownsDebug(request: unknown) {
  ...
  try {
    ...
    const lessons = await scraper.scrapeLessons(debugUrls, delayBetweenRequests, true, enableClaudeExport);
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        contentSections: lesson.cooldown ? Object.keys(lesson.cooldown).length : 0
      }
    };
