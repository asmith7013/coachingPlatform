...
export class IMScraper {
  ...
  private async removeInterferingElements(): Promise<void> {
    ...
    try {
      ...
      console.log('⏳ Page load state complete, checking for elements to remove...');
      
      // Wait for potential UserWay elements to load
      try {
        await this.page.waitForSelector('.userway_buttons_wrapper, [class*="userway"], [id*="userway"], .uw-sl', { 
          timeout: 3000 
        });
        console.log('✅ UserWay elements detected on page');
      } catch {
        console.log('ℹ️ No UserWay elements detected within timeout (this is normal if none exist)');
      }

      // Define elements to remove with descriptions for debugging
      const elementsToRemove = [
        // UserWay Accessibility controls - comprehensive selectors
        { selector: '.userway_buttons_wrapper', description: 'UserWay main wrapper' },
        { selector: '.userway_dark', description: 'UserWay dark theme elements' },
        { selector: '#userwayLstIcon', description: 'UserWay language icon' },
        { selector: '#userwayAccessibilityIcon', description: 'UserWay accessibility icon' },
        { selector: '.uwaw-dictionary-tooltip', description: 'UserWay dictionary tooltip' },
        ...
        { selector: '#uw-skip-to-main', description: 'UserWay skip to main button' },
        { selector: '#uw-enable-visibility', description: 'UserWay enable visibility button' },
        { selector: '#uw-open-accessibility', description: 'UserWay open accessibility button' },
        { selector: '[role="region"][aria-label*="Quick Accessibility"]', description: 'UserWay accessibility region' },
        { selector: '[role="region"][aria-label*="Accessibility"]', description: 'UserWay accessibility region (generic)' },
        ...
        { selector: '.im-c-figure__modal-expand-button', description: 'IM figure modal expand button' },
        { selector: '.im-c-touch-target[aria-label*="Expand"]', description: 'IM touch target expand (capitalized)' },
        { selector: '.im-c-touch-target[aria-label*="expand"]', description: 'IM touch target expand (lowercase)' },
        { selector: '.im-c-figure__expand-button', description: 'IM figure expand button' },
        { selector: 'button[aria-label*="Expand"]', description: 'Generic expand button (capitalized)' },
        { selector: 'button[aria-label*="expand"]', description: 'Generic expand button (lowercase)' },
        
        // Generic UserWay and interfering elements (keep these broad selectors last)
        { selector: '[class*="userway"]', description: 'Generic UserWay classes' },
        { selector: '[id*="userway"]', description: 'Generic UserWay IDs' },
        { selector: '[class*="uw-"]', description: 'Generic UW- prefixed classes' },
        { selector: '[id*="uw-"]', description: 'Generic UW- prefixed IDs' },
        { selector: '[class*="expand-button"]', description: 'Generic expand button classes' },
        
        // Additional potential interfering overlay elements
        { selector: '.accessibility-menu', description: 'Accessibility menu' },
        { selector: '.accessibility-widget', description: 'Accessibility widget' },
        { selector: '[role="dialog"][aria-label*="Accessibility"]', description: 'Accessibility dialog' },
        { selector: '[role="dialog"][aria-label*="Translation"]', description: 'Translation dialog' }
      ];
      ...
      for (const { selector, description } of elementsToRemove) {
        try {
          ...
          if (count > 0) {
            ...
            console.log(`   ✅ ${description}: ${count} removed - "${selector}"`);
          } else {
      ...
      console.log(`🗑️ Element removal summary: ${totalRemovedCount} elements permanently removed`);
      
      if (totalRemovedCount === 0) {
        console.log('ℹ️ No interfering elements found on this page');
      } else {
        console.log('✨ Page cleaned - interfering elements permanently removed');
      }
  ...
  private async screenshotWithPaddingAndCleanup(
    ...
    try {
      ...
      console.log(`📸 Screenshot captured with padding and cleanup: ${screenshotPath}`);
      ...
      try {
        if (originalStyles) {
          await element.evaluate((el: HTMLElement, styles: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  private async captureContentScreenshots(url: string, lessonId: string, debug = false): Promise<string[]> {
    ...
    try {
      ...
      console.log('✅ Found cooldown card, capturing clean screenshots with padding...');
      ...
      await this.screenshotWithPaddingAndCleanup(cooldownCard, cooldownScreenshotPath, SCREENSHOT_PADDING);
      screenshotPaths.push(cooldownScreenshotName);
      console.log('📸 Captured full cooldown card screenshot with padding and cleanup');
      ...
      if (await taskSection.count() > 0) {
        ...
        await this.screenshotWithPaddingAndCleanup(taskSection, taskScreenshotPath, SCREENSHOT_PADDING);
        screenshotPaths.push(taskScreenshotName);
        console.log('📸 Captured Cooldown Student Task Statement screenshot with padding and cleanup');
        ...
        for (let i = 0; i < taskMathDivs.length; i++) {
          ...
          await this.screenshotWithPaddingAndCleanup(taskMathDivs[i], mathScreenshotPath, SCREENSHOT_PADDING);
          screenshotPaths.push(mathScreenshotName);
          console.log(`📸 Captured Cooldown Task MathJax ${i + 1} with padding and cleanup`);
        }
      } else {
      ...
      if (await responseHeading.count() > 0) {
        ...
        if (await responseSection.count() > 0) {
          ...
          for (let i = 0; i < responseMathDivs.length; i++) {
            ...
            const mathScreenshotPath = `${screenshotDir}/${mathScreenshotName}`;
            await this.screenshotWithPaddingAndCleanup(responseMathDivs[i], mathScreenshotPath, SCREENSHOT_PADDING);
            screenshotPaths.push(mathScreenshotName);
            console.log(`📸 Captured Cooldown Response MathJax ${i + 1} with padding and cleanup`);
          }
        }
      } else {
      ...
      for (let i = 0; i < cooldownImages.length; i++) {
        ...
        await this.screenshotWithPaddingAndCleanup(cooldownImages[i], imageScreenshotPath, SCREENSHOT_PADDING);
        screenshotPaths.push(imageScreenshotName);
        console.log(`📸 Captured cooldown image ${i + 1} with padding and cleanup`);
      }
