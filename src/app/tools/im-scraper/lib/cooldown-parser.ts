...
export class SimplifiedCooldownParser {
  ...
  constructor() {
    ...
    this.turndownService.addRule("mathPlaceholder", {
      filter: (node) =>
        node.classList && node.classList.contains("math-placeholder"),
  ...
  private extractRawHtmlSections($: CheerioRoot): {
    ...
    const studentTaskStatement_rawHtml =
  ...
  private generateClaudeExportText(
    ...
    const responseScreenshots = screenshotRefs.filter(
      (ref) => ref.type === "response",
    );
    ...
    const otherScreenshots = screenshotRefs.filter((ref) =>
      ["image", "full"].includes(ref.type),
    );
  ...
  private extractStudentTaskStatement($: CheerioRoot) {
    ...
    $clone.find("img").each((_: number, img: any) => {
      ...
      const alt =
        $img.attr("alt") || $img.attr("data-uw-rm-alt-original") || "";
      if (url) {
        images.push({ url, alt });
      }
    });
  ...
  private processMathContent(
    ...
    $container
      ...
      .each((index: number, elem: any) => {
        ...
        const screenreaderElement = $elem
          .find(".screenreader-only, .sr-only, [aria-label]")
          .first();
        if (screenreaderElement.length > 0) {
          screenreaderText =
            screenreaderElement.text().trim() ||
            screenreaderElement.attr("aria-label");
        } else {
          // Check sibling elements
          const nearbyScreenreader = $elem
            .siblings(".screenreader-only, .sr-only")
            .first();
        ...
        $elem.replaceWith(
          `<span class="math-placeholder">${placeholder}</span>`,
        );
      });
...
export function parseCooldown(
  ...
  return parser.parseCooldownSectionWithClaudeExport(
    html,
    enableClaudeExport,
    lessonMeta,
    screenshots,
