...
export interface ClaudeProcessorActions {
  processSingle: (htmlContent: string, lessonMetadata: {
    url: string;
    grade: string;
    unit: string;
    lesson: string;
 ...
 */
export function useClaudeProcessor(): ClaudeProcessorState & ClaudeProcessorActions {
  ...
  const setProcessedLessons = useCallback((lessons: ProcessedLesson[], batchResponse?: ClaudeProcessorState['lastBatchResponse']) => {
    setState(prev => ({
      ...prev,
  ...
  const setProgress = useCallback((progress: { current: number; total: number; currentLessonUrl?: string } | null) => {
  ...
  const processSingle = useCallback(async (
    ...
    setProcessing(true);
    setError(null);
    setProgress({ current: 1, total: 1, currentLessonUrl: lessonMetadata.url });
  ...
  const processBatch = useCallback(async (lessons: IMLesson[]) => {
    ...
    if (validLessons.length === 0) {
      setError('No lessons found with Claude export content. Make sure scraping was done with Claude Export enabled.');
      return;
    }
    ...
    try {
      ...
      const batchRequest = {
        lessons: validLessons.map(lesson => ({
          htmlContent: lesson.cooldown!.claudeExport!.studentTaskStatement_rawHtml,
          lessonMetadata: {
            url: lesson.url,
            grade: lesson.grade,
            unit: lesson.unit,
            lesson: lesson.lesson,
            lessonNumber: typeof lesson.lesson === 'string' ? parseInt(lesson.lesson, 10) : undefined
          }
        }))
      };
      ...
      const updateProgress = (current: number, url?: string) => {
        setProgress({ current, total: validLessons.length, currentLessonUrl: url });
      };
      ...
      if (response.success && response.data) {
        ...
        if (response.data.totalFailed > 0) {
          ...
          setError(`Processing completed with ${response.data.totalFailed} failures: ${failedUrls.join(', ')}`);
        }
      } else {
        setError(response.error || 'Unknown error occurred');
        setProcessedLessons([]);
  ...
  const processLessonsFromScraper = useCallback(async (scrapedLessons: IMLesson[]) => {
