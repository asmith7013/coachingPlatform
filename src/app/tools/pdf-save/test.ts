  ...
  const testCases = [
    {
      ...
      expected: "introduction-to-fractions",
    },
...
export function testParseTitles() {
  ...
  testCases.forEach(({ input, expected }) => {
    ...
    console.log(
      `${passed ? "✅" : "❌"} "${input}" → [${result.join(", ")}] (expected: [${expected.join(", ")}])`,
    );
  });
}
...
export function testValidateForm() {
  ...
  testCases.forEach(
    ({ name, file, grade, unit, titles, pageCount, expected }) => {
      const result = validateForm(file, grade, unit, titles, pageCount);
      const passed = result === expected;
      console.log(
        `${passed ? "✅" : "❌"} ${name}: "${result}" (expected: "${expected}")`,
      );
    },
  );
}
