...
export class StateTestScraper {
  ...
  async login(): Promise<void> {
    ...
    await this.page.click(
      'input[type="submit"].btn-success, button[type="submit"]:has-text("Next")',
    );
    console.log("⏳ Waiting for password field...");

    // Wait for password field (multi-step login)
    await this.page.waitForSelector(SELECTORS.PASSWORD_INPUT, {
      timeout: 10000,
    });
    ...
    await this.page.click(
      'input[type="submit"].btn-success, button[type="submit"]',
    );
  ...
  async scrapePage(
    ...
    await this.page.waitForSelector(SELECTORS.QUESTION_CONTAINER, {
      timeout: 15000,
    });
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        console.log(
          `  📷 Question ${i + 1}/${count}: ${questionId} - ${standard}`,
        );
        ...
        const screenshotBuffer = await img.screenshot({
          type: "png",
          scale: "device",
        });
        ...
        throw new Error(
          `Failed to scrape question ${i + 1}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
