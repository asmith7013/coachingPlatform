...
export function VisualPlanDisplay({ visualPlan, compact = false }: VisualPlanDisplayProps) {
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.boxes}</code>
              </div>
              <div>
                <span className="text-gray-500">Value per box:</span>{' '}
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.valuePerBox}</code>
              </div>
              <div>
                <span className="text-gray-500">Total:</span>{' '}
                <code className="text-green-700 bg-green-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.total}</code>
              </div>
              <div>
                <span className="text-gray-500">Unknown:</span>{' '}
                <Badge intent="warning" size="xs">{visualPlan.unknownPosition}</Badge>
              </div>
            </div>
          </div>
        );
          ...
          <div className="space-y-2">
            <div className="bg-gray-50 rounded p-2 text-sm border border-gray-200">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-gray-600 font-medium">{visualPlan.quantityA.label}:</span>
                <code className="text-xs">{visualPlan.quantityA.values.join(' → ')}</code>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-600 font-medium">{visualPlan.quantityB.label}:</span>
                <code className="text-xs">{visualPlan.quantityB.values.join(' → ')}</code>
              </div>
            </div>
              ...
              <div className="mt-1 bg-gray-50 rounded p-2 border border-gray-200">
                {visualPlan.partialProducts.map((row, i) => (
                  <div key={i} className="flex gap-2">
                    {row.map((val, j) => (
                      <code key={j} className="text-xs bg-white px-2 py-1 rounded border border-gray-200">
                        {val}
                      </code>
                    ))}
                  </div>
            ...
            <div className="text-sm">
              ...
              <div className="flex gap-2 mt-1 flex-wrap">
                {visualPlan.markedPoints.map((pt, i) => (
                  ...
                  >
                    {pt.value}{pt.label ? ` (${pt.label})` : ''} [{pt.style}]
                  </code>
                ))}
              </div>
            </div>
              ...
              <div className="text-sm">
                ...
                <div className="flex gap-2 mt-1 flex-wrap">
                  {visualPlan.arrows.map((arr, i) => (
                    <code key={i} className="text-xs bg-purple-50 border-purple-200 text-purple-700 px-2 py-1 rounded border">
                      {arr.from} → {arr.to}{arr.label ? `: ${arr.label}` : ''}
                    </code>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              {visualPlan.rows.map((row, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  <span className="text-gray-600 font-medium w-20">{row.label}:</span>
            ...
            {visualPlan.scaleFactors && visualPlan.scaleFactors.length > 0 && (
              <div className="text-sm">
                ...
                {visualPlan.scaleFactors.map((sf, i) => (
                  <Badge key={i} intent="info" size="xs" className="ml-1">{sf}</Badge>
                ))}
              </div>
            )}
          </div>
            ...
            {visualPlan.shapes && (
              <div className="text-sm">
                ...
                {visualPlan.shapes.triangle && (
                  <Badge intent="warning" size="xs">△ = {visualPlan.shapes.triangle}</Badge>
                )}{' '}
                {visualPlan.shapes.square && (
                  <Badge intent="info" size="xs">□ = {visualPlan.shapes.square}</Badge>
                )}
              </div>
            )}
          </div>
        ...
        return (
          <div className="space-y-2">
            <div className="text-sm">
              ...
              <code className="text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.rule}</code>
            </div>
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.rows}</code>
              </div>
              <div>
                <span className="text-gray-500">Cols:</span>{' '}
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.cols}</code>
              </div>
              <div>
                <span className="text-gray-500">Unit:</span>{' '}
                <code className="text-green-700 bg-green-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.unitLabel}</code>
              </div>
            </div>
            {visualPlan.shadedRegions && visualPlan.shadedRegions.length > 0 && (
              <div className="text-sm">
                <span className="text-gray-500">Shaded Regions:</span>{' '}
                <Badge intent="info" size="xs">{visualPlan.shadedRegions.length} region(s)</Badge>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div className="space-y-2">
            <div className="text-sm">
              ...
              <Badge intent="info" size="xs">{visualPlan.shapeType}</Badge>
            </div>
            <div className="text-sm">
              <span className="text-gray-500">Dimensions:</span>{' '}
              <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">
                {visualPlan.dimensions.length} × {visualPlan.dimensions.width} × {visualPlan.dimensions.height}
              </code>
            </div>
            <div className="text-sm">
              <span className="text-gray-500">Faces:</span>{' '}
              <Badge intent="neutral" size="xs">{visualPlan.faces.length} faces</Badge>
              {visualPlan.foldLines && <Badge intent="warning" size="xs" className="ml-1">fold lines</Badge>}
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-2">
            <div className="text-sm">
              <span className="text-gray-500">Shape:</span>{' '}
              <Badge intent="info" size="xs">{visualPlan.shapeType}</Badge>
              {visualPlan.showRightAngle && <Badge intent="neutral" size="xs" className="ml-1">right angle</Badge>}
              {visualPlan.showDashedHeight && <Badge intent="neutral" size="xs" className="ml-1">dashed height</Badge>}
            </div>
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              {visualPlan.measurements.map((m, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  <span className="text-gray-600 font-medium capitalize">{m.label}:</span>
                  <code className="text-xs bg-white px-2 py-1 rounded border border-gray-200">{m.value}</code>
                  {m.position && <span className="text-gray-400 text-xs">({m.position})</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.groups}</code>
              </div>
              <div>
                <span className="text-gray-500">Items/Group:</span>{' '}
                <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.itemsPerGroup}</code>
              </div>
              <div>
                <span className="text-gray-500">Total:</span>{' '}
                <code className="text-green-700 bg-green-50 px-1.5 py-0.5 rounded text-xs">{visualPlan.totalItems}</code>
              </div>
              <div>
                <span className="text-gray-500">Style:</span>{' '}
                <Badge intent="neutral" size="xs">{visualPlan.visualType}</Badge>
              </div>
            </div>
            <div className="text-sm">
              <span className="text-gray-500">Arrangement:</span>{' '}
              <Badge intent="info" size="xs">{visualPlan.arrangement}</Badge>
            </div>
          </div>
        );
            ...
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  ...
                  <code className="text-purple-700 bg-purple-50 px-2 py-1 rounded text-lg font-bold">{visualPlan.hundreds}</code>
                </div>
                <div className="text-center">
                  <span className="text-gray-500 text-xs block">Tens</span>
                  <code className="text-blue-700 bg-blue-50 px-2 py-1 rounded text-lg font-bold">{visualPlan.tens}</code>
                </div>
                <div className="text-center">
                  <span className="text-gray-500 text-xs block">Ones</span>
                  <code className="text-green-700 bg-green-50 px-2 py-1 rounded text-lg font-bold">{visualPlan.ones}</code>
                </div>
              </div>
            </div>
            <div className="text-sm">
              <span className="text-gray-500">Operation:</span>{' '}
              <Badge intent={visualPlan.operation === 'none' ? 'neutral' : 'warning'} size="xs">
                {visualPlan.operation}
              </Badge>
              {visualPlan.showValues && <Badge intent="info" size="xs" className="ml-1">show values</Badge>}
            </div>
          </div>
        );
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <Badge intent="neutral" size="xs">{visualPlan.axisLabel}</Badge>
              </div>
              <div>
                <span className="text-gray-500">Range:</span>{' '}
            ...
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              <div className="grid grid-cols-5 gap-2 text-sm text-center">
                <div>
                  ...
                  <code className="text-blue-700 font-bold">{visualPlan.min}</code>
                </div>
                <div>
                  <span className="text-gray-500 text-xs block">Q1</span>
                  <code className="text-blue-700 font-bold">{visualPlan.q1}</code>
                </div>
                <div>
                  <span className="text-gray-500 text-xs block">Median</span>
                  <code className="text-purple-700 font-bold">{visualPlan.median}</code>
                </div>
                <div>
                  <span className="text-gray-500 text-xs block">Q3</span>
                  <code className="text-blue-700 font-bold">{visualPlan.q3}</code>
                </div>
                <div>
                  <span className="text-gray-500 text-xs block">Max</span>
                  <code className="text-blue-700 font-bold">{visualPlan.max}</code>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {visualPlan.outliers && visualPlan.outliers.length > 0 && (
                <div>
                  ...
                  <Badge intent="warning" size="xs">{visualPlan.outliers.join(', ')}</Badge>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              {visualPlan.categories.map((cat, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  <span className="text-gray-600 font-medium w-20 truncate">{cat}:</span>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <Badge intent="info" size="xs">{visualPlan.orientation}</Badge>
              </div>
              <div>
                <span className="text-gray-500">Axis:</span>{' '}
                <Badge intent="neutral" size="xs">{visualPlan.axisLabel}</Badge>
        ...
        return (
          <div className="space-y-2">
            <div className="text-sm">
              ...
              <Badge intent="info" size="xs">{visualPlan.levels.length}</Badge>
            </div>
            <div className="bg-gray-50 rounded p-2 border border-gray-200">
              {visualPlan.levels.map((level, i) => (
                <div key={i} className="text-sm mb-1">
                  <span className="text-gray-600 font-medium">Level {i + 1}:</span>{' '}
                  {level.outcomes.map((outcome, j) => (
                    <span key={j}>
                      ...
                      {j < level.outcomes.length - 1 && <span className="text-gray-400 mx-1">|</span>}
                    </span>
                  ))}
                </div>
              ))}
            ...
            {visualPlan.highlightPath && (
              <div className="text-sm">
                <span className="text-gray-500">Highlight:</span>{' '}
                <Badge intent="warning" size="xs">{visualPlan.highlightPath.join(' → ')}</Badge>
              </div>
            )}
          </div>
        );
          ...
          <div className="space-y-2">
            ...
            <div className="text-sm">
              ...
              {visualPlan.labeledParts.map((part, i) => (
                <Badge key={i} intent="info" size="xs" className="ml-1">{part}</Badge>
              ))}
            </div>
          </div>
        );
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 rounded p-2 border border-blue-200">
                <span className="text-xs text-blue-600 font-medium block mb-1">Drawing</span>
                {visualPlan.drawingMeasurements.map((m, i) => (
                  <div key={i} className="text-sm">
                    {m.label}: <code className="text-blue-700">{m.value} {m.unit}</code>
                  </div>
                ))}
              </div>
              <div className="bg-green-50 rounded p-2 border border-green-200">
                <span className="text-xs text-green-600 font-medium block mb-1">Actual</span>
                {visualPlan.actualMeasurements.map((m, i) => (
                  <div key={i} className="text-sm">
                    {m.label}: <code className="text-green-700">{m.value} {m.unit}</code>
                  </div>
                ))}
              </div>
            </div>
            <div className="text-sm">
              ...
              <Badge intent="neutral" size="xs">{visualPlan.drawingType}</Badge>
            </div>
          </div>
        );
      ...
      case 'scaled-figures':
        return (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 rounded p-2 border border-blue-200">
                <span className="text-xs text-blue-600 font-medium block mb-1">Original</span>
              ...
              <div className="bg-green-50 rounded p-2 border border-green-200">
                <span className="text-xs text-green-600 font-medium block mb-1">Copy</span>
            ...
            {visualPlan.shapeType && (
              <div className="text-sm">
                <span className="text-gray-500">Shape:</span>{' '}
                <Badge intent="neutral" size="xs">{visualPlan.shapeType}</Badge>
              </div>
            )}
          </div>
        );
              ...
              <div className="text-sm">
                ...
                <div className="flex gap-1 mt-1 flex-wrap">
                  {visualPlan.elements.map((el, i) => (
                    <Badge key={i} intent="neutral" size="xs">{el}</Badge>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="text-sm">
                ...
                <div className="flex gap-1 mt-1 flex-wrap">
                  {visualPlan.annotations.map((ann, i) => (
                    <Badge key={i} intent="info" size="xs">{ann}</Badge>
                  ))}
                </div>
              </div>
            )}
