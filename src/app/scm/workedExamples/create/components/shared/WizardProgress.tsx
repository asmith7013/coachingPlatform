        ...
        <nav aria-label="Progress">
          <ol role="list" className="flex relative items-stretch">
            {STEPS.map((step, index) => {
              ...
              const hoverBgClass = isCompleted
                ? "group-hover:bg-blue-50"
                : isNext
                  ? "group-hover:bg-green-50"
                  : "";
              const hoverTextClass = isCompleted
                ? "group-hover:text-blue-700"
                : isNext
                  ? "group-hover:text-green-700"
                  : "";
              const hoverSubtextClass = isCompleted
                ? "group-hover:text-blue-600"
                : isNext
                  ? "group-hover:text-green-600"
                  : "";
              const hoverCircleClass = isNext
                ? "group-hover:border-green-500 group-hover:bg-green-50"
                : "";
              const hoverNumberClass = isNext
                ? "group-hover:text-green-600"
                : "";
              const hoverSeparatorClass = isCompleted
                ? "group-hover/step:text-blue-300"
                : isNext
                    ...
                    <span className="flex-shrink-0">
                      ...
                      >
                        {isCompleted ? (
                          ...
                          <span
                            className={`transition-colors duration-150 ${hoverNumberClass}`}
                          >
                            {step.number}
                          </span>
                        )}
              ...
              return (
                <li
                  key={step.number}
                  className={`relative flex-1 ${clickable ? "group/step" : ""}`}
                >
                  {actuallyClickable ? (
                    <button
                    ...
                    <div
                      className={`${clickable ? "group cursor-pointer" : ""} ${isCurrent ? "" : !clickable ? "opacity-60" : ""}`}
                    >
                      {StepContent}
                    </div>
                  )}
