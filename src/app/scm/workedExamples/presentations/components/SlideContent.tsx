 ...
 */
function extractBodyBackgroundStyles(
  html: string,
  ...
  styleString.split(";").forEach((rule) => {
    ...
    if (
      property &&
      value &&
      backgroundProps.some((bp) => property.toLowerCase().startsWith(bp))
    ) {
...
export function SlideContent({ slide, currentRevealed }: SlideContentProps) {
  ...
  const backgroundStyles = useMemo(
    () => extractBodyBackgroundStyles(slide.htmlContent),
    [slide.htmlContent],
  );
    ...
    <>
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: generateAnimationCSS(currentRevealed),
        }}
      />
