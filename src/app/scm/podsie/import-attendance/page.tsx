...
export default function ImportAttendancePage() {
  ...
  const [currentSyncingSection, setCurrentSyncingSection] = useState<string | null>(null);
      ...
      if (response.success && response.items) {
        ...
        const sectionsWithGroupId = response.items
          ...
          .map((s) => ({
            ...
            displayName: s.teacher ? `${s.classSection} (${s.teacher})` : String(s.classSection),
          }));
  ...
  const handleSelectAll = (school: string, schoolSections: SectionOption[]) => {
    ...
    if (allSelected) {
      ...
      setSelectedSections((prev) => prev.filter((id) => !schoolSectionIds.includes(id)));
    } else {
      // Select all sections in this school
      setSelectedSections((prev) => [...new Set([...prev, ...schoolSectionIds])]);
    }
  };
  ...
  const handleFetchAndImport = async () => {
    ...
    const sectionsToSync = sections.filter((s) => selectedSections.includes(s.id));
    ...
    try {
      ...
      // Calculate totals
      const totalProcessed = syncResults.reduce((sum, r) => sum + r.totalProcessed, 0);
      const totalCreated = syncResults.reduce((sum, r) => sum + r.created, 0);
      const totalUpdated = syncResults.reduce((sum, r) => sum + r.updated, 0);
      const totalNotTracked = syncResults.reduce((sum, r) => sum + r.notTracked, 0);
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Automatically fetch and import attendance data from Podsie for selected sections.
        </p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="flex items-end gap-6 flex-wrap">
            ...
            <button
              ...
              disabled={isLoading || selectedSections.length === 0 || !startDate}
            ...
            >
              {isLoading ? (
                <span className="flex items-center gap-2">
                  ...
                  {currentSyncingSection ? `Syncing ${currentSyncingSection}...` : "Syncing..."}
                </span>
              ) : (
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
              ...
              <div className="bg-green-50 border border-green-200 rounded-md p-4">
                <div className="text-2xl font-bold text-green-700">{totalProcessed}</div>
                <div className="text-sm text-green-600">Total Processed</div>
              </div>
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                <div className="text-2xl font-bold text-blue-700">{totalCreated}</div>
                <div className="text-sm text-blue-600">Created</div>
              </div>
              <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
                <div className="text-2xl font-bold text-yellow-700">{totalUpdated}</div>
                <div className="text-sm text-yellow-600">Updated</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-md p-4">
                <div className="text-2xl font-bold text-gray-700">{totalNotTracked}</div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-gray-700">
            <li>Select one or more sections from the checkboxes above (all sections selected by default)</li>
            <li>Choose a start date (defaults to yesterday)</li>
            <li>Click &ldquo;Fetch & Import&rdquo; to automatically:
            ...
            <li>Review the results to see how many records were processed per section</li>
          </ol>
