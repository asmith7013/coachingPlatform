...
export default function StateTestPage() {
  ...
  const handleScrape = () => {
    ...
    startTransition(async () => {
      const res = await scrapeStateTestPage(
        url,
        grade,
        examYear,
        examTitle || `${examYear} State Test`,
      );
      setResult(res);
  ...
  const handleReplaceScreenshots = (year: string) => {
    ...
    startTransition(async () => {
      ...
      addLog(
        `Grade 6: ${res.grade6.replaced}/${res.grade6.total} ${res.grade6.success ? "SUCCESS" : "FAILED"}`,
      );
      addLog(
        `Grade 7: ${res.grade7.replaced}/${res.grade7.total} ${res.grade7.success ? "SUCCESS" : "FAILED"}`,
      );
      addLog(
        `Grade 8: ${res.grade8.replaced}/${res.grade8.total} ${res.grade8.success ? "SUCCESS" : "FAILED"}`,
      );
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        {result && (
          ...
          >
            {result.success
              ? `Successfully scraped ${result.count} questions`
              : result.error}
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Replace existing screenshots (with answers) with new answer-free
          versions from local folders.
        </p>
        <div className="flex gap-3">
          <button
        ...
        {replaceResult && (
          <div className="mt-3 p-3 rounded-md bg-gray-50 text-sm">
            ...
            <p>
              Grade 6: {replaceResult.result.grade6.replaced}/
              {replaceResult.result.grade6.total}{" "}
              {replaceResult.result.grade6.success ? "✓" : "✗"}
            </p>
            <p>
              Grade 7: {replaceResult.result.grade7.replaced}/
              {replaceResult.result.grade7.total}{" "}
              {replaceResult.result.grade7.success ? "✓" : "✗"}
            </p>
            <p>
              Grade 8: {replaceResult.result.grade8.replaced}/
              {replaceResult.result.grade8.total}{" "}
              {replaceResult.result.grade8.success ? "✓" : "✗"}
            </p>
            {/* Show errors if any */}
            {(replaceResult.result.grade6.errors.length > 0 ||
              ...
              <div className="mt-2 p-2 bg-red-50 text-red-800 rounded text-xs">
                ...
                {replaceResult.result.grade6.errors.map((e, i) => (
                  <p key={`g6-${i}`}>G6: {e}</p>
                ))}
                {replaceResult.result.grade7.errors.map((e, i) => (
                  <p key={`g7-${i}`}>G7: {e}</p>
                ))}
                {replaceResult.result.grade8.errors.map((e, i) => (
                  <p key={`g8-${i}`}>G8: {e}</p>
                ))}
              </div>
            )}
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        {questions.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm">
            No questions loaded. Click &quot;Load All&quot; or scrape a new
            page.
          </p>
        )}
      </div>
