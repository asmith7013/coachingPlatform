...
export function ColumnLessonsRow({
  ...
  if (config.isCompleteColumn) {
    const completeTooltipContent =
      config.studentNames.length > 0 ? config.studentNames.join("\n") : "";
    ...
    if (!showStudentNames && completeTooltipContent) {
      return (
        <Tooltip content={completeTooltipContent}>{completeContent}</Tooltip>
      );
    }
          ...
          >
            {lessons.length > 0 ? (
              lessons.map((lesson, lessonIndex) => {
                ...
                const lessonContent = (
                  <div className="flex flex-col justify-start items-start px-2 py-1.5 w-full h-full">
                    <span
                      className={`text-[9px] ${styles.text} leading-none text-left`}
                    >
                      {lesson.lessonName}
                    </span>
                    <StudentIcons
                ...
                if (!showStudentNames && tooltipContent) {
                  return (
                    <div
                      ...
                      style={{ width: `${lessonWidthPercent}%` }}
                    >
              ...
              <div className="flex-1 flex items-center justify-center">
                <span className={`text-[9px] ${styles.text} opacity-50`}>
                  —
                </span>
              </div>
            )}
