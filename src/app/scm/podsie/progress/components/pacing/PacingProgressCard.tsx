...
function PresentationChartLineOutlineIcon({
  className,
}: {
  className?: string;
}) {
...
export function PacingProgressCard({
  ...
  const [internalShowStudentNames, setInternalShowStudentNames] =
    useState(false);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
      ...
      <div className="flex items-center justify-between mb-2">
        {customHeader || (
          <h3 className="text-lg font-semibold text-gray-900">
            Unit {selectedUnit} Pace &amp; Progress
          </h3>
        )}
        {!hideToggle && (
      ...
      */}
      ...
      {totalStudents > 0 &&
        (() => {
          const farBehindPercent = Math.round(
            (students.farBehind.length / totalStudents) * 100,
          );
          const behindPercent = Math.round(
            (students.behind.length / totalStudents) * 100,
          );
          const onPacePercent = Math.round(
            (students.onTrack.length / totalStudents) * 100,
          );
          const aheadPercent = Math.round(
            (students.ahead.length / totalStudents) * 100,
          );
          const farAheadPercent = Math.round(
            (students.farAhead.length / totalStudents) * 100,
          );
          const completePercent = Math.round(
            (completeCount / totalStudents) * 100,
          );
              ...
              <div className="flex h-6">
                {segments.map((segment, index) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-xs ${segment.text} font-bold whitespace-nowrap overflow-hidden`}
                      >
                        {segment.percent}%
                      </span>
                    </div>
