...
function calculateColumnWidths(
  sectionsByZone: Map<string, UnitSectionInfo[]>,
  totalLessons: number,
) {
  const MIN_WIDTH_PERCENT = 8;

  const zonesWithWidth = Array.from(sectionsByZone.entries()).map(
    ([zone, sections]) => {
      const zoneLessons = sections.reduce((sum, s) => sum + s.lessonCount, 0);
      const rawPercent =
        totalLessons > 0
          ? (zoneLessons / totalLessons) * 100
          : 100 / sectionsByZone.size;
      return {
        zone,
        sections,
  ...
  const totalAdjusted = zonesWithWidth.reduce(
    (sum, z) => sum + z.adjustedPercent,
    0,
  );
...
function buildColumnConfigs(
  ...
  const columnConfigs: ColumnConfig[] = normalizedZones.map(
    (zoneData, index) => {
      const totalStudents = zoneData.sections.reduce(
      ...
      return {
        ...
        isFixedWidth: false,
        isLastColumn:
          index === normalizedZones.length - 1 && !hasCompletedStudents,
    ...
    <div>
      ...
      <div className="flex h-7 rounded-t-lg overflow-hidden border border-gray-200 border-b-0">
        {columnConfigs.map((config) => (
          <ColumnHeader key={`header-${config.zone}`} config={config} />
        ))}
      </div>
      ...
      <div className="flex h-12 border border-gray-200 border-t-0 border-b-0">
        {columnConfigs.map((config) => (
          <ColumnSectionRow key={`sections-${config.zone}`} config={config} />
        ))}
      </div>

      {/* Lessons sub-bar */}
      <div
        className={`flex h-auto min-h-[1.75rem] overflow-hidden border border-gray-200 border-t-0 ${!showStudentNames ? "rounded-b-lg" : ""}`}
      >
        {columnConfigs.map((config) => (
          <ColumnLessonsRow
