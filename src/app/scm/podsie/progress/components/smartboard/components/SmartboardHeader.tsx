...
export function SmartboardHeader({
  ...
  return (
    <div
      className={`bg-indigo-900 rounded-t-xl flex items-center justify-between ${isFullscreen ? "p-8" : "p-4"}`}
    >
      <div className="flex items-center gap-4">
        <div
          className={`bg-teal-500 text-white rounded-lg font-bold ${isFullscreen ? "px-6 py-3" : "px-4 py-2"} ${textSizes.badge}`}
        >
          Mini Goal
        </div>
        <div
          className={`bg-indigo-700 text-white rounded-lg font-semibold ${isFullscreen ? "px-6 py-3" : "px-4 py-2"} ${textSizes.unit}`}
        >
        ...
        {isEditMode ? (
          ...
          <div
            className={`bg-indigo-600 text-white rounded-lg ${isFullscreen ? "px-6 py-3" : "px-4 py-2"} ${textSizes.date}`}
          >
            By {formattedDueDate}
          </div>
        )}
      ...
      <div className="flex items-center gap-4">
        <div
          className={`bg-white text-indigo-900 rounded-lg font-bold ${isFullscreen ? "px-6 py-3" : "px-4 py-2"} ${textSizes.count}`}
        >
        ...
        {onSyncAll && (
          ...
          >
            <ArrowPathIcon
              className={`${isFullscreen ? "w-8 h-8" : "w-6 h-6"} text-white ${syncingAll ? "animate-spin" : ""}`}
            />
          </button>
        )}
        {!isEditMode && activeYoutubeUrl && onPlayVideo && (
          ...
          >
            <PlayIcon
              className={
                isFullscreen ? "w-8 h-8 text-white" : "w-6 h-6 text-white"
              }
            />
          </button>
        )}
        {(onTextSizeDecrease || onTextSizeIncrease) && (
          <div className="flex items-center gap-1">
            ...
            <span
              className={`font-bold text-white ${isFullscreen ? "text-base" : "text-sm"} w-6 text-center`}
            >
              A
            </span>
            {onTextSizeIncrease && (
