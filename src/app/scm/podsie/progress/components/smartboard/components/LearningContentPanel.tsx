...
export function LearningContentPanel({
  ...
  return (
    <div className={`bg-indigo-100 rounded-xl p-5 border-4 border-indigo-300 ${getTextSize("w-72", "w-80", "w-96", "w-[28rem]")}`}>
      <h3 className={`text-indigo-900 font-bold mb-4 text-center ${getTextSize("text-base", "text-lg", "text-xl", "text-2xl")}`}>
        What We&apos;re Learning
      </h3>
      {isEditMode ? (
        <div>
          ...
          />
          <p className={`text-indigo-600 mt-2 ${getTextSize("text-[10px]", "text-xs", "text-sm", "text-base")}`}>
            Enter one item per line. Use &quot;-&quot; or just text.
          </p>
        </div>
      ) : (
        <ul className={`space-y-3 text-indigo-900 ${getTextSize("text-sm", "text-base", "text-lg", "text-xl")}`}>
