...
function getAttendanceBackground(
  status: "present" | "late" | "absent" | "not-tracked" | null,
): string {
  if (!status) return "";
...
function ActivityBadge({
  ...
  return (
    <span
      className={`px-2 py-1 ${colors[type]} rounded font-bold text-xs leading-none`}
    >
      {count}
    </span>
  );
            ...
            students.map((student) => (
              <tr key={student.studentId} className="hover:bg-gray-50">
                ...
                {schoolDays.map((date) => {
                  ...
                  const tooltipText =
                    dayData?.completedLessons &&
                    dayData.completedLessons.length > 0
                      ? dayData.completedLessons
                          .map((lesson) => {
                            // Format lesson type
                            const lessonTypeLabel =
                              lesson.lessonType === "rampUp"
                                ? "Ramp Up"
                  ...
                  const attendanceBackground = getAttendanceBackground(
                      ...
                      >
                        <div className="flex justify-center gap-1 min-h-[24px]">
                          {dayData && dayData.lessons > 0 && (
                            <ActivityBadge
                              count={dayData.lessons}
                              type="lesson"
                            />
                          )}
                          {showRampUps && dayData && dayData.rampUps > 0 && (
                            <ActivityBadge
                              count={dayData.rampUps}
                              type="rampUp"
                            />
                          )}
                        </div>
