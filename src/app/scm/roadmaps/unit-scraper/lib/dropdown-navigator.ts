...
const SELECTORS = {
  ...
  ROADMAP_INPUT:
    '#unitSelector .p-inputgroup:has(.p-inputgroup-addon:has-text("Roadmap")) input.p-autocomplete-input',
  UNIT_INPUT:
    '#unitSelector .p-inputgroup:has(.p-inputgroup-addon:has-text("Unit")) input.p-autocomplete-input',
...
export async function getAvailableRoadmaps(page: Page): Promise<string[]> {
  ...
  const options = await page
    .locator(SELECTORS.AUTOCOMPLETE_ITEM)
    .allTextContents();
...
export async function selectRoadmap(
  ...
  const selectedValue = await page
    .locator(SELECTORS.ROADMAP_INPUT)
    .inputValue();
  if (selectedValue !== roadmap) {
    throw new Error(
      `Failed to select roadmap. Expected: ${roadmap}, Got: ${selectedValue}`,
    );
  }
...
export async function selectUnit(page: Page, unitName: string): Promise<void> {
  ...
  if (selectedValue !== unitName) {
    throw new Error(
      `Failed to select unit. Expected: ${unitName}, Got: ${selectedValue}`,
    );
  }
