...
export function AppearsInSection({ appearsIn }: AppearsInSectionProps) {
  ...
  const hasContent = appearsIn.asTarget?.length > 0 ||
  ...
  const firstTargetUnit = appearsIn.asTarget && appearsIn.asTarget.length > 0
  ...
  appearsIn.asTarget?.forEach(unit => {
    ...
    appearances.push({ unitNumber: unit.unitNumber, unitTitle: unit.unitTitle, type: 'target' });
  ...
  appearsIn.asEssential?.forEach(relatedSkill => {
    relatedSkill.units.forEach(unit => {
      const appearances = gradeMap.get(unit.grade) || [];
      appearances.push({ unitNumber: unit.unitNumber, unitTitle: unit.unitTitle, type: 'essential' });
      gradeMap.set(unit.grade, appearances);
    });
  });
  ...
  appearsIn.asHelpful?.forEach(relatedSkill => {
    relatedSkill.units.forEach(unit => {
      const appearances = gradeMap.get(unit.grade) || [];
      appearances.push({ unitNumber: unit.unitNumber, unitTitle: unit.unitTitle, type: 'helpful' });
      gradeMap.set(unit.grade, appearances);
    });
  });
  ...
  appearsIn.asSupport?.forEach(unit => {
    const appearances = gradeMap.get(unit.grade) || [];
    appearances.push({ unitNumber: unit.unitNumber, unitTitle: unit.unitTitle, type: 'support' });
    gradeMap.set(unit.grade, appearances);
  });

  // Sort grades and build grade data list
  const sortedGrades = Array.from(gradeMap.keys()).sort((a, b) => getGradeNum(a) - getGradeNum(b));
