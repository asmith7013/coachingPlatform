      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            {incompleteCount > 0 && (
              <span className={`${colorClasses.badgeBg} px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1`}>
                <span>○</span>
          ...
          {items.length > 0 ? (
            items.map((item, index) =>
              renderItem ? renderItem(item, index) : (
                <div key={index} className={`${colorClasses.itemBg} ${colorClasses.itemBorder} border rounded-lg p-3`}>
                  <h5 className={`font-medium ${colorClasses.itemText} text-sm`}>
                    {item.label}
                  </h5>
                  {item.details && (
