...
const STATUS_OPTIONS: Array<{ value: string; label: string }> = [
  ...
  {
    value: "Attempted But Not Mastered",
    label: "⏳ Attempted But Not Mastered",
  },
];
...
export default function AssessmentHistoryPage() {
  ...
  const {
    ...
    error: zearnError,
    refetch: refetchZearn,
  } = useZearnCompletions();
  const {
    data: podsieData,
    loading: podsieLoading,
    error: podsieError,
  } = usePodsieCompletions();
  ...
  const masteryPercentage =
    totalAttempts > 0 ? (masteredAttempts / totalAttempts) * 100 : 0;
  ...
  const handleCopyShoutouts = async () => {
    ...
    const studentCounts = new Map<
      string,
      { zearn: number; roadmaps: number; podsie: number }
    >();
    ...
    for (const row of filteredZearnData) {
      ...
      const current = studentCounts.get(firstName) || {
        zearn: 0,
        roadmaps: 0,
        podsie: 0,
      };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <DeprecationModal
        links={[
          {
            label: "Go to History on Podsie",
            url: "https://www.podsie.org/teacher/sandbox/history",
          },
        ]}
      />
      <div className="mx-auto p-6" style={{ maxWidth: "1600px" }}>
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 mb-6">
          ...
          <p className="text-gray-600">
            View all student assessment attempts and progress.
          </p>
        </div>
                ...
                >
                  {SECTION_OPTIONS.map((option) => {
                    ...
                    const isFirstOfGrade =
                      !prevOption || prevOption.grade !== option.grade;

                    if (isFirstOfGrade && option.grade) {
                      // Render optgroup header
                      const gradeOptions = SECTION_OPTIONS.filter(
                        (opt) => opt.grade === option.grade,
                      );
                      return (
        ...
        >
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-500 transition-colors">
            ...
            />
            <label htmlFor="zearn-file-upload" className="cursor-pointer">
            ...
            <div
              className={`mt-4 rounded-lg p-4 ${
                zearnImportResult.success && zearnImportResult.imported > 0
                  ...
                  : zearnImportResult.success &&
                      zearnImportResult.imported === 0
                ...
                <div className="text-xs text-gray-600 mt-2 max-h-20 overflow-y-auto">
                  ...
                  {zearnImportResult.errors.length > 5 && (
                    <div className="italic">
                      ...and {zearnImportResult.errors.length - 5} more
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="flex justify-end gap-3 mt-4">
            <Button intent="secondary" onClick={handleCloseZearnImport}>
              {zearnImportResult?.success && zearnImportResult.imported > 0
                ? "Done"
                : "Cancel"}
            </Button>
