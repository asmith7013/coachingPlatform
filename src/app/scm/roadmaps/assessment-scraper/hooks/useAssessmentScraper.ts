...
import {
  scrapeAndUpdateAssessmentData,
  scrapeAndUpdateAllSections,
} from "../actions/scrape-and-update";
...
export function useAssessmentScraper() {
  ...
  const [batchResults, setBatchResults] = useState<BatchScraperResults | null>(
    null,
  );
  ...
  const scrapeAndUpdateBatch = async (credentials: {
    email: string;
    password: string;
  }) => {
    setIsLoading(true);
