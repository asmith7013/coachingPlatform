 ...
 */
export async function extractSkillData(
  page: Page,
  url: string,
): Promise<SkillData> {
  try {
    ...
    const skillChallengeCriteria = await extractFieldsetContent(
      page,
      "Skill Challenge Criteria",
    );
    const essentialQuestion = await extractFieldsetContent(
      page,
      "Essential Question",
    );
    ...
    const teacherStudentStrategies = await extractSectionContent(
      page,
      "Teacher/Student Strategies:",
    );
    const modelsAndManipulatives = await extractSectionContent(
      page,
      "Models and Manipulatives:",
    );
    const questionsToHelp = await extractSectionContent(
      page,
      "Questions to Help Students Get Unstuck:",
    );
    const discussionQuestions = await extractSectionContent(
      page,
      "Discussion Questions:",
    );
    const commonMisconceptions = await extractSectionContent(
      page,
      "Common Misconceptions:",
    );
    const additionalResources = await extractSectionContent(
      page,
    ...
    const practiceProblems = await screenshotPracticeProblems(
      page,
      skillNumber,
    );
    console.log(
      `🔍 [EXTRACTOR] Practice problems extracted: ${practiceProblems.length}`,
    );
    if (practiceProblems.length > 0) {
      console.log(
        `🔍 [EXTRACTOR] First practice problem:`,
        JSON.stringify(practiceProblems[0], null, 2),
      );
    }
    ...
    console.log(
      `🔍 [EXTRACTOR] Returning skill data with ${skillData.practiceProblems.length} practice problems`,
    );
    console.log(`🔍 [EXTRACTOR] Video URL: ${skillData.videoUrl || "(empty)"}`);
    console.log(
      `🔍 [EXTRACTOR] Images with context: ${skillData.imagesWithContext.length}`,
    );
    console.log(
      `📚 [EXTRACTOR] Vocabulary terms: ${skillData.vocabulary.length}`,
    );
    if (skillData.vocabulary.length > 0) {
      console.log(
        `📚 [EXTRACTOR] Vocabulary: ${skillData.vocabulary.map((v) => v.term).join(", ")}`,
      );
    }
    console.log(
      `📋 [EXTRACTOR] Standards extracted: ${skillData.standards ? "Yes" : "No"}`,
    );
    if (skillData.standards) {
      const standardsCount = skillData.standards
        .split("\n\n")
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        if (await accordion.isVisible()) {
          ...
          console.log(
            `🔄 Expanding accordion ${i + 1}/${count}: ${accordionText?.trim().substring(0, 50)}...`,
          );
 ...
 */
async function extractFieldsetContent(
  page: Page,
  legendText: string,
): Promise<string> {
  try {
    ...
    const fieldset = page.locator(
      `fieldset:has(legend:has-text("${legendText}"))`,
    );
    ...
    console.warn(
      `Could not extract fieldset content for ${legendText}:`,
      error,
    );
    return "";
  }
...
async function extractStandardsContent(page: Page): Promise<string> {
  try {
    ...
    const headerLink = page
      .locator('.p-accordion-header-link:has-text("Standards")')
      .first();
    ...
    try {
      ...
      console.warn(
        `Standards content area #${contentId} not attached after 3s`,
      );
      return "";
    }
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        const legendText = await fieldset
          .locator("legend .p-fieldset-legend-text")
        ...
        const contentText = await fieldset
          .locator(".p-fieldset-content")
          .textContent({ timeout: 2000 });
...
async function extractPrimerHtml(page: Page): Promise<string> {
  try {
    ...
    console.log(
      `📄 Extracted complete primer HTML (${primerHtml.length} characters)`,
    );
    return primerHtml;
  } catch (error) {
...
async function extractVocabulary(
  ...
  try {
    ...
    const headerLink = page
      .locator('.p-accordion-header-link:has-text("Vocabulary")')
      .first();
    ...
    try {
      ...
      console.warn(
        `Vocabulary content area #${contentId} not attached after 3s`,
      );
      return [];
    }
    ...
    for (let i = 0; i < count; i++) {
      try {
        const fieldset = fieldsets.nth(i);
        const term = await fieldset
          .locator("legend .p-fieldset-legend-text")
          .textContent({ timeout: 2000 });
        const definition = await fieldset
          .locator(".p-fieldset-content")
          .textContent({ timeout: 2000 });
    ...
    console.log(
      `📚 Extracted ${vocabularyTerms.length} vocabulary terms with definitions`,
    );
    if (vocabularyTerms.length > 0) {
      console.log(
        `📚 Sample: ${vocabularyTerms[0].term}: ${vocabularyTerms[0].definition.substring(0, 50)}...`,
      );
    }
 ...
 */
async function extractImagesWithContext(
  page: Page,
  ...
  const sections = [
    ...
    {
      name: "teacherStudentStrategies",
      headerText: "Teacher/Student Strategies:",
    },
    { name: "modelsAndManipulatives", headerText: "Models and Manipulatives:" },
    {
      name: "questionsToHelp",
      headerText: "Questions to Help Students Get Unstuck:",
    },
  ...
  for (const section of sections) {
    try {
      ...
      if ((await header.count()) > 0) {
        ...
        const sectionImages = await page.evaluate(
      ...
      console.warn(
        `Could not extract images from section ${section.name}:`,
        error,
      );
    }
  }
...
async function navigateToPracticeProblemsTab(page: Page): Promise<boolean> {
  try {
    ...
    const practiceTab = page.locator(
      'span.p-tabview-title:text("Practice Problems")',
    );
    ...
    if (tabCount > 0) {
      ...
      console.log(
        "🔍 [DEBUG] Clicked Practice Problems tab, waiting for content...",
      );
      ...
      const visibleContent = await page
        ...
        .count();
      console.log(
        `🔍 [DEBUG] Found ${visibleContent} visible tabpanel(s) after navigation`,
      );
...
async function extractAndSaveWorkedExampleVideo(
  ...
  try {
    ...
    const additionalLessonsAccordion = page.locator(
      ROADMAPS_CONSTANTS.SELECTORS.ADDITIONAL_LESSONS_ACCORDION,
    );
    ...
    const isExpanded =
      await additionalLessonsAccordion.getAttribute("aria-expanded");
    if (isExpanded === "false") {
    ...
    const videoLink = page.locator(
      ROADMAPS_CONSTANTS.SELECTORS.WORKED_EXAMPLE_VIDEO_LINK,
    );
    ...
    await page.waitForSelector(ROADMAPS_CONSTANTS.SELECTORS.VIDEO_DIALOG, {
      timeout: 5000,
    });
    ...
    if ((await videoSource.count()) === 0) {
      ...
      await page
        .locator(ROADMAPS_CONSTANTS.SELECTORS.VIDEO_DIALOG_CLOSE)
        .click();
      return "";
    }
    ...
    try {
      ...
      const existingVideo = blobs.find(
        (blob) => blob.pathname === blobFileName,
      );

      if (existingVideo) {
        console.log(
          `♻️ Video already exists in Vercel Blob, reusing: ${existingVideo.url}`,
        );
    ...
    try {
      ...
      if (errorMessage.includes("already exists")) {
        console.log(
          `♻️ Upload failed (blob exists), fetching existing video URL...`,
        );
        try {
          const { blobs } = await list({ prefix });
          const existingVideo = blobs.find(
    ...
    try {
      const dialogClose = page.locator(
        ROADMAPS_CONSTANTS.SELECTORS.VIDEO_DIALOG_CLOSE,
      );
...
async function screenshotPracticeProblems(
  ...
  try {
    ...
    console.log(
      `🔍 Looking for divs with id starting with "${skillNumber}_practice_"`,
    );
    ...
    await page.evaluate(() => {
      ...
      const userGuidingLauncher = document.getElementById(
    ...
    if (count === 0) {
      ...
      console.log(
        "🔍 [DEBUG] No problems found with expected pattern. Checking page content...",
      );
      const allDivsWithIds = await page.locator('div[id*="practice"]').count();
      console.log(
        `🔍 [DEBUG] Found ${allDivsWithIds} divs with "practice" in their id`,
      );

      // Get a sample of IDs to see the actual pattern
      const sampleIds = await page
        .locator('div[id*="practice"]')
        .evaluateAll((divs) =>
          divs
            .slice(0, 5)
            .map((div) => div.id)
    ...
    console.log(
      `⏳ Waiting 5 seconds for all ${count} practice problems to fully render...`,
    );
    ...
    console.log(
      `✅ Successfully screenshot ${problems.length}/${count} practice problems`,
    );
    return problems;
  } catch (error) {
