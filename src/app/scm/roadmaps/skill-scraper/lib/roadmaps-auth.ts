...
export async function debugPasswordField(page: Page): Promise<void> {
  ...
  if (count > 0) {
    ...
    console.log(
      `🔍 Field state - Visible: ${isVisible}, Enabled: ${isEnabled}, Editable: ${isEditable}`,
    );
    ...
    if (boundingBox) {
      const elementAtPoint = await page
        .locator("*")
        .filter({
...
export async function authenticateRoadmaps(
  ...
  try {
    ...
    if ((await loginButton.count()) > 0) {
      ...
      await page.waitForSelector(ROADMAPS_CONSTANTS.EMAIL_FIELD_SELECTOR, {
        timeout: 10000,
      });
    ...
    if (!passwordValue || passwordValue !== credentials.password) {
      ...
      if (!passwordValue) {
        ...
        const altPasswordField = page.locator(
          ROADMAPS_CONSTANTS.ALT_PASSWORD_SELECTOR,
        );
        if ((await altPasswordField.count()) > 0) {
          ...
          if (!altValue) {
            throw new Error(
              "Password field could not be filled with any method",
            );
          }
        } else {
