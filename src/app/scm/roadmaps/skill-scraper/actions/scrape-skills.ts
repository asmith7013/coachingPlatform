  ...
  try {
    ...
    for (let i = 0; i < skillUrls.length; i++) {
      ...
      try {
        ...
        console.log(
          `🔍 [SCRAPER] Received skill data with ${skillData.practiceProblems?.length || 0} practice problems`,
        );
        ...
        if (skillData.success) {
          ...
          console.log(
            `💾 Saving skill ${skillData.skillNumber} to database...`,
          );
          const saveResult = await updateRoadmapsSkillContent(
            skillData as Parameters<typeof updateRoadmapsSkillContent>[0],
          );
          ...
          if (saveResult.success) {
            ...
            console.error(
              `❌ Failed to save skill ${skillData.skillNumber}: ${saveResult.error}`,
            );
          }
        } else {
    ...
    const totalPracticeProblems = skills.reduce(
      (sum, skill) => sum + (skill.practiceProblems?.length || 0),
      0,
    );
    console.log(
      `🔍 [SCRAPER] Total practice problems across all skills: ${totalPracticeProblems}`,
    );
    ...
    console.log(
      `🔍 [SCRAPER] Response contains ${response.skills.length} skills`,
    );
    response.skills.forEach((skill, idx) => {
      console.log(
        `🔍 [SCRAPER] Skill ${idx + 1}: ${skill.skillNumber} - ${skill.practiceProblems?.length || 0} practice problems`,
      );
    });
    ...
    console.log(
      `🔍 [SCRAPER] After validation: ${validatedResponse.skills.length} skills, first has ${validatedResponse.skills[0]?.practiceProblems?.length || 0} practice problems`,
    );
...
export async function validateRoadmapsCredentials(credentials: unknown) {
  ...
  try {
    ...
    const validatedCredentials =
      RoadmapsScrapingRequestZodSchema.shape.credentials.parse(credentials);
    ...
    if (authResult.success) {
      ...
      console.log(
        "✅ Successfully navigated to skill page after authentication",
      );
    }
