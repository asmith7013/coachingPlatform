...
export function SkillProgressionTable({
  lessons,
  onLessonClick,
  masteredSkills = [],
  selectedLessonId = null,
}: SkillProgressionTableProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 mb-6">
          ...
          <tbody>
            ...
            {showPrerequisites && (
              <tr className="border-b-2 border-gray-400">
                ...
                {allTargetSkills.map((skill, skillIndex) => {
                  const colorScheme =
                    SKILL_COLORS[skillIndex % SKILL_COLORS.length];
                  const skillData = skillsData.get(skill);
                    ...
                    >
                      <div className="text-[10px] space-y-2">
                        ...
                        {essentialSkills.length > 0 && (
                          <div>
                            <div className="font-semibold mb-1 text-skill-essential">
                              Essential:
                            </div>
                            <div className="space-y-0.5">
                              {essentialSkills.map((essSkill) => {
                                const skillNum =
                                  typeof essSkill === "string"
                                    ? essSkill
                                    : essSkill.skillNumber;
                                const skillTitle =
                                  typeof essSkill === "object"
                                    ? essSkill.title
                                    : undefined;
                        ...
                        {helpfulSkills.length > 0 && (
                          <div>
                            <div className="font-semibold mb-1 text-skill-helpful">
                              Helpful:
                            </div>
                            <div className="space-y-0.5">
                              {helpfulSkills.map((helpSkill) => {
                                const skillNum =
                                  typeof helpSkill === "string"
                                    ? helpSkill
                                    : helpSkill.skillNumber;
                                const skillTitle =
                                  typeof helpSkill === "object"
                                    ? helpSkill.title
                                    : undefined;
                                const isSkillMastered =
                                  masteredSkills.includes(skillNum);
            ...
            {lessons.map((lesson, lessonIndex) => {
              ...
              return (
                ...
                >
                  <td
                    className={`sticky left-0 bg-inherit px-3 py-2 font-medium text-gray-700 text-center ${
                      isSelected
                        ? "bg-gray-300"
                        : lessonIndex % 2 === 0
                  ...
                  <td
                    className={`sticky left-[60px] bg-inherit px-3 py-2 font-medium text-gray-700 text-center ${
                      isSelected
                        ? "bg-gray-300"
                        : lessonIndex % 2 === 0
                          ? "bg-gray-50 group-hover:bg-blue-100"
      ...
      <div className="mt-3 text-xs text-gray-600">
        Showing {allTargetSkills.length} target skill
        {allTargetSkills.length !== 1 ? "s" : ""} across {lessons.length} lesson
        {lessons.length !== 1 ? "s" : ""}
      </div>
    </div>
  );
