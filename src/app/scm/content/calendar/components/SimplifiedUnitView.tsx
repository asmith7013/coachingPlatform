    ...
    <div className="bg-white rounded-lg shadow overflow-hidden border-l-4 border-gray-400">
      ...
      <div className="px-4 py-3 font-semibold text-sm flex items-center justify-between bg-gray-600 text-white">
        ...
        <div className="flex items-center gap-1.5 text-xs font-normal">
          ...
          >
            {unit.startDate
              ? new Date(unit.startDate + "T12:00:00").toLocaleDateString("en-US", { month: "2-digit", day: "2-digit" })
              : "mm/dd"}
          </button>
          <input
            ref={startDateRef}
          ...
          >
            {unit.endDate
              ? new Date(unit.endDate + "T12:00:00").toLocaleDateString("en-US", { month: "2-digit", day: "2-digit" })
              : "mm/dd"}
          </button>
          <input
            ref={endDateRef}
      ...
      <div className="px-4 py-1.5 bg-gray-200 border-b border-gray-300 flex items-center justify-between text-xs text-gray-600">
        <div className="flex items-center gap-3">
          <span>{unit.sections.reduce((sum, s) => sum + (s.lessons?.length || 0), 0)} lessons</span>
          <span className="text-gray-300">·</span>
          <span>{new Set(unit.sections.map(s => s.sectionId)).size} sections</span>
        </div>
        {onAddLesson && (
          <button
            onClick={onAddLesson}
            className="flex items-center gap-1 px-2 py-0.5 text-blue-600 hover:bg-blue-50 rounded cursor-pointer"
          >
        ...
        {entries.map((entry) => {
          ...
          return (
            ...
            >
              ...
              >
                <span className="font-medium text-sm" style={{ color: color.base }}>{entry.label}</span>
                {onOpenSubsections && entry.allParentLessons.length > 0 && !entry.isSplit && (
                  <button
                    onClick={() => onOpenSubsections(unit.unitKey, entry.parentSectionId, `Section ${entry.parentSectionId}`, entry.allParentLessons)}
              ...
              <div className="px-4 py-2">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    {entry.lessons.map((lesson, index) => (
                      <span
                        ...
                        style={{ backgroundColor: color.light, color: color.base }}
                        title={lesson.lessonName}
                      >
                        {isRampUp ? `Ramp Up ${index + 1}` : `Lesson ${lesson.lessonNumber}`}
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div
                  ...
                  style={{ backgroundColor: isSelected ? color.light : undefined }}
                >
                  {/* Days */}
                  <span className="text-xs min-w-[55px]" style={{ color: allocatedDays !== null && allocatedDays >= section.lessonCount ? color.base : allocatedDays !== null ? "#DC2626" : "#9CA3AF" }}>
                    {allocatedDays !== null ? `${allocatedDays}d / ${section.lessonCount}L` : `- / ${section.lessonCount}L`}
                  </span>
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: isSelectingStart || section.startDate ? color.base : "white",
                      color: isSelectingStart || section.startDate ? "white" : color.base,
                      border: !isSelectingStart && !section.startDate ? `1px solid ${color.base}` : "1px solid transparent",
                    }}
                  >
                    {section.startDate
                  ...
                  <button
                    ...
                    className="text-xs px-2 py-1 rounded cursor-pointer whitespace-nowrap"
                    style={{
                      backgroundColor: isSelectingEnd || section.endDate ? color.base : "white",
                      color: isSelectingEnd || section.endDate ? "white" : color.base,
                      border: !isSelectingEnd && !section.endDate ? `1px solid ${color.base}` : "1px solid transparent",
                    }}
                  >
                    {section.endDate
