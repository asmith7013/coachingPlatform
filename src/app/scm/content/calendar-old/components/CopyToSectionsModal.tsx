...
export function CopyToSectionsModal({
  ...
  const unitsWithDates = unitSchedules.filter(
    (u) => u.startDate || u.endDate || u.sections.some((s) => s.startDate || s.endDate)
  );
  ...
  const allSectionsSelected = otherSections.every(s => copyTargets.has(`${s.school}|${s.classSection}`));
  const allUnitsSelected = unitsWithDates.length > 0 && unitsWithDates.every(u => selectedUnits.has(u.unitNumber));
  ...
  return (
      ...
      title={
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-normal">
            Copy from <span className="font-medium">{sourceSection.school} - {sourceSection.classSection}</span>
          </p>
        </div>
      }
      size="md"
    ...
    >
      <div className="px-6 py-4 space-y-4">
        {/* Unit Selection */}
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">Select Units to Copy</h4>
          {unitsWithDates.length === 0 ? (
            <p className="text-sm text-gray-500 italic">No units have dates set yet.</p>
          ) : (
            <div className="space-y-1 max-h-32 overflow-y-auto border border-gray-200 rounded-lg p-2">
              ...
              <label className="flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-50 cursor-pointer">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedUnits(new Set(unitsWithDates.map(u => u.unitNumber)));
                    } else {
                ...
                />
                <span className="text-sm font-medium text-gray-700">Select All Units</span>
              </label>
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">Select Target Sections</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {section.teacher && (
                      <span className="text-xs text-gray-500 ml-2">({section.teacher})</span>
                    )}
                  </div>
                </label>
              );
      ...
      <div className="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
        <span className="text-sm text-gray-500">
          {selectedUnits.size} unit{selectedUnits.size !== 1 ? 's' : ''} → {copyTargets.size} section{copyTargets.size !== 1 ? 's' : ''}
        </span>
        <div className="flex gap-3">
          ...
          <button
            onClick={handleCopy}
            disabled={copying || copyTargets.size === 0 || selectedUnits.size === 0}
