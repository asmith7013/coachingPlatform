...
export function QuestionsByUnitTable({
  ...
  const totalUnmatched = unitDistribution?.unmatchedQuestions.reduce((sum, q) => sum + q.count, 0) || 0;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200">
            ...
            <th className="text-center py-1.5 font-medium text-gray-600 text-xs">Total %</th>
          </tr>
        </thead>
        <tbody>
          {unitDistribution ? (
            unitDistribution.rows.map((row) => {
              const maxMcPct = Math.max(...unitDistribution.rows.map(r => r.mcPct));
              const maxCrPct = Math.max(...unitDistribution.rows.map(r => r.crPct));
              const getMcColor = (pct: number) => {
                if (pct === 0) return "";
                if (pct >= maxMcPct * 0.8) return "bg-green-100 text-green-800";
                if (pct >= maxMcPct * 0.5) return "bg-yellow-100 text-yellow-800";
                return "bg-red-100 text-red-800";
              };
              const getCrColor = (pct: number) => {
                if (pct === 0) return "";
                if (pct >= maxCrPct * 0.8) return "bg-green-100 text-green-800";
                if (pct >= maxCrPct * 0.5) return "bg-yellow-100 text-yellow-800";
                return "bg-red-100 text-red-800";
              };
              const totalQuestions = unitDistribution.totalMC + unitDistribution.totalCR;
              const combinedPct = totalQuestions > 0
                ? Math.round(((row.mc + row.cr) / totalQuestions) * 100)
                : 0;
              const maxCombinedPct = Math.max(...unitDistribution.rows.map(r =>
                totalQuestions > 0 ? Math.round(((r.mc + r.cr) / totalQuestions) * 100) : 0
              ));
              const getCombinedColor = (pct: number) => {
                if (pct === 0) return "";
                if (pct >= maxCombinedPct * 0.8) return "bg-green-100 text-green-800";
                if (pct >= maxCombinedPct * 0.5) return "bg-yellow-100 text-yellow-800";
                return "bg-red-100 text-red-800";
              };
              return (
                <tr
                ...
                >
                  ...
                  <td className={`text-center py-1.5 text-xs rounded ${getMcColor(row.mcPct)}`}>{row.mcPct}%</td>
                  <td className={`text-center py-1.5 text-xs rounded ${getCrColor(row.crPct)}`}>{row.crPct}%</td>
                  <td className={`text-center py-1.5 text-xs font-medium rounded ${getCombinedColor(combinedPct)}`}>{combinedPct}%</td>
                </tr>
              );
            })
          ) : (
      ...
      {unitDistribution && unitDistribution.unmatchedQuestions.length > 0 && (
        <div className="mt-4 pt-3 border-t border-gray-200">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-semibold text-amber-700">Not in curriculum</span>
