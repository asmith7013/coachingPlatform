...
import {
  getAllRoadmapsStudents,
  getRoadmapsStudentPerformance,
} from "@actions/scm/student/student-performance";
    ...
    const loadLessons = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          const rawLessons = Array.isArray(result.data)
            ? result.data
            : result.data.items || [];
          const lessonsWithStringIds = rawLessons.map(
            (lesson: Record<string, unknown>): RoadmapsLesson => ({
              _id: (lesson._id as string)?.toString() || (lesson._id as string),
              ownerIds: (lesson.ownerIds as string[]) || [],
              title:
                (lesson.title as string) || (lesson.lessonName as string) || "",
              ...
              suggestedTargetSkills:
                (lesson.suggestedTargetSkills as string[]) || [],
              ...
              practiceProblems:
                (lesson.practiceProblems as Array<{
                  problemNumber: number;
                  screenshotUrl: string;
                  scrapedAt: string;
                }>) || [],
              scrapedAt:
                (lesson.scrapedAt as string) || new Date().toISOString(),
    ...
    const loadStudentData = async () => {
      try {
        ...
        if (
          ...
          setError(
            "Student data is too large to load. Please contact support.",
          );
        }
      } finally {
    ...
    return (
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6">
          IM Lessons & Roadmaps Skills Explorer
        </h1>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-center">
            <div className="text-yellow-600 mr-2">ℹ️</div>
            <div className="text-yellow-800">
              No lessons found. Please add some IM lessons to get started.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <p className="text-gray-600">
            Select a student to see their skill mastery levels, then choose a
            lesson to explore how target skills connect to their essential
            prerequisites and helpful supporting skills.
          </p>
        </div>
              ...
              {selectedStudentData && !studentDataLoading && (
                <div className="mt-4 bg-purple-50 border border-purple-200 rounded-lg p-3">
                  ...
                  <div className="text-xs text-purple-700">
                    Skills Tracked:{" "}
                    {selectedStudentData.skillPerformances.length}
                  </div>
                </div>
              )}
            </div>
