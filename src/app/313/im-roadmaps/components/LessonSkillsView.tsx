...
function TargetSkillDisplay({
  targetSkill,
  isSelected,
}: TargetSkillDisplayProps) {
  const masteryStyles = getMasteryStyles(targetSkill.masteryStatus);

  return (
    <div
      className={`bg-blue-50 border border-blue-200 rounded-lg p-4 ${masteryStyles.containerClass} ${
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3
              className={`text-lg font-bold text-blue-900 ${masteryStyles.textClass}`}
            >
...
function SkillCard({ skill, colorScheme }: SkillCardProps) {
  ...
  return (
    <div
      className={`${colorClasses.bg} ${colorClasses.border} border rounded-lg p-3 ${masteryStyles.containerClass}`}
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2 flex-1">
          ...
          <h5
            className={`font-medium ${colorClasses.text} text-sm ${masteryStyles.textClass}`}
          >
...
export function LessonSkillsView({
  lessonId,
  lessons,
  studentData,
}: LessonSkillsViewProps) {
  const [roadmapsSkills, setRoadmapsSkills] = useState<RoadmapsSkill[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTargetSkillIndex, _setSelectedTargetSkillIndex] = useState<
    number | null
  >(null);
    ...
    const loadRoadmapsSkills = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          const rawSkills = Array.isArray(result.data)
            ? result.data
            : (result.data as any)?.items || result.data || []; // eslint-disable-line @typescript-eslint/no-explicit-any
          const skillsWithStringIds = rawSkills.map(
            (skill: any): RoadmapsSkill => ({
  ...
  const getSkillDetails = (skillTexts: string[]) => {
    return skillTexts.map((skillText) => {
      ...
      const roadmapsSkill = roadmapsSkills.find(
        (skill) => skill.skillNumber === skillId,
      );
      ...
      return {
        ...
        description:
          roadmapsSkill?.description ||
          "Skill details not found in roadmaps collection",
  ...
  if (error) {
    return (
      <div className="text-red-600 py-8">Error loading skills: {error}</div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          {selectedLesson.grade} {selectedLesson.unit} - Section{" "}
          {selectedLesson.section}, Lesson {selectedLesson.lesson}
        </p>
        ...
        {targetSkills.length > 0 ? (
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
            <p className="text-yellow-800">
              No target skills specified for this lesson.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="mt-8">
          ...
          <div className="space-y-3">
            {selectedLesson.vocabulary.map((term, index) => {
              ...
              const word =
                colonIndex > -1 ? term.substring(0, colonIndex).trim() : term;
              const definition =
                colonIndex > -1 ? term.substring(colonIndex + 1).trim() : "";
