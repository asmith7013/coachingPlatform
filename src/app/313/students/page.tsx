...
export default function StudentViewerPage() {
  ...
  const availableTeachers = useMemo(() => {
    ...
    return (
      districtMapping[selectedDistrict as keyof typeof districtMapping]
        ?.teachers || []
    );
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      if (selectedDistrict) {
        const teacherInDistrict = availableTeachers.includes(
          student.teacher as Teachers313Type,
        );
        const sectionInDistrict = availableSections.includes(
          student.section as Sections313Type,
        );
      ...
      if (searchQuery.trim()) {
        ...
        const matchesSearch =
          fullName.includes(query) || studentId.includes(query);
  ...
  const handleGradeLevelChange = useCallback(
    async (studentId: string, newGradeLevel: string) => {
      try {
  ...
  const handleSectionChange = useCallback(
    async (studentId: string, newSection: string) => {
      try {
        await updateWithToast(studentId, {
          section: newSection as Sections313Type,
        });
      } catch (error) {
  ...
  const handleTeacherChange = useCallback(
    async (studentId: string, newTeacher: string) => {
      try {
        await updateWithToast(studentId, {
          teacher: newTeacher as Teachers313Type,
        });
      } catch (error) {
