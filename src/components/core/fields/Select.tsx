...
const select = tv({
  slots: {
    ...
    placeholder: ["block truncate text-gray-400 italic"],
  ...
  variants: {
    ...
    error: {
      true: {
        trigger: "border-red-500 focus:ring-red-500 focus:border-red-500",
      },
      false: {},
    },
    active: {
        ...
        >
          ...
          >
            {multiple ? (
              ...
              <span className={styles.value()}>
                {options.find((option) => option.value === safeValue)
                  ?.label || (
          ...
          <Listbox.Options className={styles.options()}>
            {options.map((option) => (
              <Listbox.Option
                ...
                value={option.value}
                className={({ active }) =>
                  cn(styles.option(), active ? "bg-indigo-50" : "")
                }
              >
                {({ selected, active }) => (
                  <>
