...
const label = tv({
  base: ["font-medium", textColors.default],
  variants: {
    textSize: {
...
export function Label({ children, className, textSize, ...props }: LabelProps) {
  return (
    <label className={cn(label({ textSize }), className)} {...props}>
      {children}
    </label>
  );
}
