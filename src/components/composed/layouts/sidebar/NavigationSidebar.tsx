...
import {
  ...
  useSignOut,
} from "@/hooks/auth/useAuthenticatedUser";
...
const sidebar = tv({
  slots: {
    ...
    mobilePanel:
      "relative mr-16 flex w-full max-w-xs flex-1 transform transition duration-300 ease-in-out data-closed:-translate-x-full",
    mobileCloseButton:
      "absolute top-0 left-full flex w-16 justify-center pt-5 duration-300 ease-in-out data-closed:opacity-0",
    ...
    userProfileSection:
      "flex h-16 shrink-0 items-center border-b border-gray-200 pb-4",
    userMenuButton:
      "flex w-full items-center rounded-md p-2 text-left hover:bg-gray-50",
    ...
    userMenuItem:
      "flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900",
    ...
    // Settings at bottom
    settingsItem:
      "group -mx-2 flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold text-gray-700 hover:bg-gray-50 hover:text-indigo-600",
...
export function NavigationSidebar({
        ...
        {teams.map((team) => (
          <li key={team.name}>
            <Link
              ...
              className={cn(
                team.current
                  ? styles.teamItemCurrent()
                  : styles.teamItemDefault(),
                styles.teamItem(),
              )}
            >
              <span
                className={cn(
                  team.current
                    ? styles.teamBadgeCurrent()
                    : styles.teamBadgeDefault(),
                  styles.teamBadge(),
                )}
              >
                {team.initial}
      ...
      <div className={styles.sidebarContent()}>
        ...
        <nav className={styles.nav()}>
          <ul role="list" className={styles.navList()}>
            ...
            {showTeams && teams.length > 0 && <li>{renderTeams()}</li>}
          </ul>
        </nav>

        {/* Settings - fixed at bottom, outside scrollable area */}
        <div className="mt-auto pt-4 border-t border-gray-200">
          <Link href="/settings" className={styles.settingsItem()}>
            <svg
  ...
  const renderMobileDialog = () => (
    ...
    >
      <DialogBackdrop transition className={styles.mobileBackdrop()} />

      <div className={styles.mobileContainer()}>
        <DialogPanel transition className={styles.mobilePanel()}>
          <TransitionChild>
