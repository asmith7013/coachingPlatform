...
export const pageHeader = tv({
  slots: {
    root: ["flex flex-col lg:flex-row lg:items-center lg:justify-between"],
    content: ["min-w-0 flex-1"],
    title: ["font-bold sm:truncate text-xl", textColors.default],
    subtitle: ["text-base", textColors.muted],
    meta: ["flex flex-col sm:mt-0 sm:flex-row sm:flex-wrap"],
    metaItem: ["flex items-center text-base", textColors.muted],
    metaIcon: ["size-5 shrink-0", textColors.muted],
    actions: ["flex lg:mt-0 lg:ml-4"],
    actionItem: ["flex"],
    actionIcon: ["-ml-0.5 size-5"],
    ...
    menuItem: ["block w-full px-4 py-2 text-sm focus:outline-none"],
  },
  variants: {
    padding: {
      sm: {
          ...
          <div className={styles.meta()}>
            {meta.map((item, index) => (
              <div key={index} className={styles.metaItem()}>
                {item.icon && (
                  <item.icon className={styles.metaIcon()} aria-hidden="true" />
                )}
                {item.value}
              </div>
            ))}
