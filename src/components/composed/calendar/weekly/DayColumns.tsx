...
const dayColumns = tv({
  slots: {
    container:
      "sticky top-0 z-30 flex-none bg-white shadow-sm ring-1 ring-black/5 sm:pr-8",
    ...
    mobileDayActive:
      "mt-1 flex size-8 items-center justify-center rounded-full bg-indigo-600 font-semibold text-white",
    desktopGrid:
      "-mr-px hidden grid-cols-7 divide-x divide-gray-100 border-r border-gray-100 text-sm/6 text-gray-500 sm:grid",
        ...
        <div className={styles.mobileGrid()}>
          {weekDays.map((date, index) => (
            <button key={index} type="button" className={styles.mobileButton()}>
              ...
              <span
                className={
                  isToday(date) ? styles.mobileDayActive() : styles.mobileDay()
                }
              >
                {date.getDate()}
              </span>
