...
import {
  AI_PROMPTS,
  createMasterSchedulePrompt,
} from "@ui/data-import/schema-templates";
...
export function DataImportDialog({
  ...
  const { items: allStaff, isLoading: _isLoadingStaff } = useNYCPSStaffList({
  ...
  // Mutations
  const { mutate: createStaffMutation, isPending: creatingStaff } =
    useStandardMutation(
  ...
  const { mutate: createVisitMutation, isPending: creatingVisit } =
    useStandardMutation(
  ...
  const {
    mutate: createBellScheduleMutation,
    isPending: creatingBellSchedule,
  } = useStandardMutation(
  ...
  const dataTypeOptions = [
    {
      ...
      title: "Staff Members",
      description:
        "Add teachers, administrators, and other staff to this school",
      icon: UserGroupIcon,
  ...
  const handleDataTypeSelect = useCallback(
    (dataType: DataType) => {
      ...
      if (dataType === "masterSchedule" && schoolStaff.length === 0) {
        alert(
          "Please add staff members to this school before creating a master schedule.",
        );
        return;
      }
    ...
    (jsonString: string) => {
      ...
      if (selectedDataType === "staff") {
        ...
        const dataArray = Array.isArray(result.data)
          ? result.data
          : [result.data];
        const scheduleWithSchool = dataArray.map(
          (schedule: BellScheduleInput) => ({
            ...schedule,
            school: school._id,
            owners: [],
          }),
        ...
        setImportData((prev) => ({
          ...prev,
          bellSchedules: scheduleWithSchool,
        }));
        ...
        setImportData((prev) => ({
          ...prev,
          masterSchedule: scheduleWithSchool,
        }));
      }
  ...
  const handleCreateData = useCallback(async () => {
    ...
    if (selectedDataType === "staff" && importData.staff.length > 0) {
      createStaffMutation([importData.staff, school._id], {
        ...
        onError: (error) => {
          alert(
            `Failed to add staff: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        },
      });
      ...
      try {
        ...
        alert(
          `Failed to add visits: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    } else if (
      selectedDataType === "bellSchedules" &&
      importData.bellSchedules.length > 0
    ) {
      try {
        ...
        alert(
          `Failed to add bell schedules: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    } else if (
      selectedDataType === "masterSchedule" &&
      importData.masterSchedule.length > 0
    ) {
      try {
        ...
        alert(
          `Failed to add master schedule: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
  }, [
        ...
        {dataTypeOptions.map((option) => (
          ...
          >
            <div className="text-center">
              <div
                className={`mx-auto flex items-center justify-center h-12 w-12 rounded-full ${option.color} mb-3`}
              >
            ...
            {selectedDataType === "masterSchedule" &&
              schoolStaff.length === 0 && (
                <Alert intent="warning" className="mt-4">
                  <Alert.Description>
                    No staff members found for this school. Please add staff
                    members first using the &quot;Staff Members&quot; option
                    above.
                  </Alert.Description>
                </Alert>
              )}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="text-xs font-medium text-gray-900">
                        ...
                        <span className="ml-2 text-green-600">
                          ({schoolStaff.length} staff members available)
                        </span>
                      </h5>
                      <Button
                        appearance="outline"
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border rounded-lg p-3">
              ...
              <p className="text-xs text-gray-500 mt-1">
                {currentData.length} {selectedDataType} record(s) ready to be
                added
              </p>
            </div>
          </div>
            ...
            <>
              {importData[selectedDataType].length > 0 ? (
                <Button
                  ...
                  loading={
                    creatingStaff || creatingVisit || creatingBellSchedule
                  }
                >
                  Add{" "}
                  {selectedDataType === "staff"
