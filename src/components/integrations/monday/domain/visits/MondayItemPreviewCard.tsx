  ...
  return (
    <div className="border-l-4 border-yellow-400 pl-4 py-2 mt-4">
      <Text className="text-sm font-medium mb-2">
        Select an owner for this visit
      </Text>
      <div className="flex gap-2">
        <ReferenceSelect
          ...
          value={selectedOwner}
          onChange={(value) =>
            setSelectedOwner(Array.isArray(value) ? value[0] : value)
          }
...
export function MondayItemPreviewCard({
  ...
  const { original, transformed, valid, isDuplicate, missingRequired, errors } =
    preview;
  ...
  const statusBadges = useMemo(() => {
    ...
    valid,
    isDuplicate,
    preview.existingItem,
    needsOwner,
    hasAutoAssignedOwner,
  ]);
  ...
  const handleToggle = () => {
    ...
    const isSelectableWithOwner =
      !valid && needsOwner && missingRequired.length === 1;
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">{statusBadges}</div>
            </div>
          </div>
        </div>
