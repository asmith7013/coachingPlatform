...
import {
  ...
  type ZearnImportRecordInput,
} from "@zod-schema/scm/zearn-import";
import { Student } from "@/lib/schema/zod-schema/scm/student/student";
import {
  parseZearnData,
  type ParsedStudentData,
} from "../utils/parseZearnData";
...
export function useZearnBatchData() {
  ...
  const [storageData, setStorageData] = useLocalStorage<LocalStorageData>(
    STORAGE_KEY,
    {},
  );
  ...
  const checkNameMatch = useCallback(
    (student: Student, parsedData: ParsedStudentData | null) => {
      ...
      const zearnParsedName = `${parsedData.firstName} ${parsedData.lastName}`
        .toLowerCase()
        .trim();

      // Exact match
      if (
        studentFullName === zearnName ||
        studentFullName === zearnParsedName
      ) {
        return {
          isMatched: true,
      ...
      const allWordsMatch = studentWords.every((word) =>
        zearnWords.some(
          (zearnWord) => zearnWord.includes(word) || word.includes(zearnWord),
        ),
      );
          ...
          if (result.success && result.items) {
            ...
            const students: Student[] = result.items.map(
              (item: Record<string, unknown>) => ({
                ...
                lastAssessmentDate: item.lastAssessmentDate as
                  | string
                  | undefined,
            ...
            console.warn(
              `Failed to load students for section ${section}:`,
              result.error,
            );
          }
        }
  ...
  const handleRawDataChange = useCallback(
    (studentId: string, rawData: string) => {
  ...
  const handleManualNameCorrection = useCallback(
    (studentId: string, correctedName: string) => {
      const student = students.find((s) => s._id === studentId);
      const studentSection = student?.section || selectedSections[0] || "";
  ...
  const handleBatchUpload = useCallback(async () => {
    ...
    if (validRows.length === 0) {
      setUploadError(
        "No valid data to upload. All students must have parsed data with matching names.",
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        alert(
          `Successfully imported ${result.data.rowsAdded} records for ${validRows.length} students!`,
        );
  ...
  const handleClearSections = useCallback(() => {
    if (
      window.confirm(
        `Clear all data for sections: ${selectedSections.join(", ")}?`,
      )
    ) {
      setStorageData((prev) => {
        const newData = { ...prev };
