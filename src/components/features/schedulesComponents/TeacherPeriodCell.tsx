...
export function TeacherPeriodCell({
  ...
  const periodBlock = schedule?.timeBlocks?.find(block => block.periodNumber === period);
  ...
  if (visitBlock) {
    ...
    return (
      <div 
        className={cn("h-16 flex flex-col items-center justify-center rounded", styling.className, className)}
        onClick={onClick}
      >
  ...
  const cellClasses = getTeacherCellStyle(isSelected, periodBlock?.activityType);

  return (
    <div 
      className={cn("h-16 rounded cursor-pointer transition-all hover:opacity-80 flex items-center justify-center", cellClasses, className)}
      onClick={onClick}
    >
      <div className="text-center">
        <div className="font-medium text-sm">
          {periodBlock?.activityType === 'lunch' ? 'LUNCH' :
           periodBlock?.activityType === 'prep' ? 'PREP' :
           periodBlock?.className || 'Available'}
        </div>
        {periodBlock?.room && periodBlock?.activityType !== 'lunch' && periodBlock?.activityType !== 'prep' && (
