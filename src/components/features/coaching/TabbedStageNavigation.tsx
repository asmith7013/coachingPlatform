...
export function TabbedStageNavigation({
  ...
  const handleKeyDown = (event: React.KeyboardEvent, stageNumber: number) => {
    ...
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      event.preventDefault();
      const currentIndex = stages.findIndex(stage => stage.number === currentStage);
  ...
  return (
    <nav className={cn(styles.container(), className)} aria-label="Stage navigation">
          ...
          return (
            ...
            >
              <div className={styles.tabContent()}>
                {/* Stage number circle */}
                <div className={cn(styles.stageNumber(), getStageNumberStyle(stage, isActive))}>
                  {stage.number}
                </div>
...
export const defaultStageConfiguration: Omit<StageInfo, 'isComplete' | 'isValid'>[] = [
  {
    number: 1,
    title: 'Needs & Focus',
