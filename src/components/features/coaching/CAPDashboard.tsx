...
} from "@components/domain/coaching";
...
export function CoachingActionPlanDashboard({
  className,
}: CoachingActionPlanDashboardProps) {
  ...
  const handleStatusChange = useCallback(
    async (planId: string, newStatus: PlanStatus, reason?: string) => {
      try {
        const result = await updateCoachingActionPlanStatus(
          planId,
          newStatus,
          reason,
        );
        ...
        const errorMessage = handleClientError(
          error,
          "updateCoachingActionPlanStatus",
        );
        showToast({
          title: "Error",
          description: errorMessage,
  ...
  // Early returns
  if (isLoading)
    return <Text textSize="base">Loading coaching action plans...</Text>;
  if (error)
    return (
