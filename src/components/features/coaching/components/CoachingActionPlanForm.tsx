...
export function CoachingActionPlanForm({
  ...
  const academicYearOptions = AcademicYearZod.options.map((value: string) => ({
    value,
    label: value,
  }));
  const statusOptions = CoachingActionPlanStatusZod.options.map(
  ...
  const renderSectionFields = (section: string) => {
    switch (section) {
        ...
        return (
          <>
            <form.Field name="title">
              {(field) => <Input fieldApi={field} label="Title" required />}
            </form.Field>
      ...
      case "needs":
        return (
          <>
            <form.Field name="needsAndFocus.ipgCoreAction">
              {(field) => <Input fieldApi={field} label="IPG Core Action" />}
            </form.Field>
            <form.Field name="needsAndFocus.ipgSubCategory">
              {(field) => <Input fieldApi={field} label="IPG Sub Category" />}
            </form.Field>
            <form.Field name="needsAndFocus.rationale">
              {(field) => <Textarea fieldApi={field} label="Rationale" />}
            </form.Field>
          </>
        );
      case "goal":
        return (
          <>
            <form.Field name="goal.description">
              {(field) => (
                <Textarea fieldApi={field} label="Goal Description" />
              )}
            </form.Field>
          </>
        );
        ...
        return (
          <>
            <form.Field name="startDate">
              {(field) => (
                <Input fieldApi={field} label="Start Date" type="date" />
              )}
            </form.Field>
            <form.Field name="endDate">
              {(field) => (
                <Input fieldApi={field} label="End Date" type="date" />
              )}
            </form.Field>
            <form.Field name="cycleLength">
              {(field) => (
                <Input fieldApi={field} label="Cycle Length" type="number" />
              )}
            </form.Field>
          </>
        );
      case "weekly":
        return (
          <form.Field name="weeklyPlans">
            {(field) => <Textarea fieldApi={field} label="Weekly Plans" />}
          </form.Field>
        );
      case "implementation":
        return (
          <form.Field name="implementationRecords">
            {(field) => (
              <Textarea fieldApi={field} label="Implementation Records" />
            )}
          </form.Field>
        );
      case "analysis":
        return (
          <form.Field name="endOfCycleAnalysis">
            {(field) => (
              <Textarea fieldApi={field} label="End of Cycle Analysis" />
            )}
          </form.Field>
        );
      default:
  ...
  return (
    <Card>
      <Card.Header>
        <Heading level="h3">
          {mode === "create"
            ? "Create New Coaching Action Plan"
            : "Edit Coaching Action Plan"}
        </Heading>
        ...
        >
          <fieldset disabled={isSubmitting} className="space-y-4">
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-200">
              ...
              <Button type="submit" appearance="solid" disabled={isSubmitting}>
                {isSubmitting
                  ? "⏳ Saving..."
                  : mode === "create"
                    ? "Create Plan"
