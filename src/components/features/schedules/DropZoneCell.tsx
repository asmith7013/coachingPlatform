...
const dropZoneCell = tv({
  slots: {
    container: "h-16 p-1",
    emptyDropZone:
      "h-full border-2 border-dashed border-gray-300 rounded flex items-center justify-center text-gray-500 text-xs",
    buttonArea: "space-y-1",

    // Updated styled badge slots
    scheduledBadge:
      "px-2 py-1 rounded-md border text-xs font-medium flex items-center space-x-1 min-h-[2rem]",
...
export function DropZoneCell({
  ...
  const isHalfAvailableForSelected = (
  ...
  const fullPeriodItem = dropZoneItems.find(
    (item) => item.portion === ScheduleAssignment.FULL_PERIOD,
  );
  const firstHalfItem = dropZoneItems.find(
    (item) => item.portion === ScheduleAssignment.FIRST_HALF,
  );
  const secondHalfItem = dropZoneItems.find(
    (item) => item.portion === ScheduleAssignment.SECOND_HALF,
  );
    ...
    return (
      <div className={styles.buttonArea()}>
        ...
        {firstHalfItem ? (
          ...
          <button
            onClick={() =>
              handlePeriodPortionSelect(ScheduleAssignment.FIRST_HALF)
            }
            className={cn(
        ...
        {secondHalfItem ? (
          ...
          <button
            onClick={() =>
              handlePeriodPortionSelect(ScheduleAssignment.SECOND_HALF)
            }
            className={cn(
    ...
    <div className={cn(styles.container(), className)}>
      {dropZoneItems.length > 0 ? (
        ...
        <div className="h-full flex gap-1 p-1">
          ...
          <button
            onClick={() =>
              handlePeriodPortionSelect(ScheduleAssignment.FULL_PERIOD)
            }
            className={cn(
              "flex-1 flex items-center justify-center rounded text-xs font-medium cursor-pointer transition-colors",
        ...
        <div className={styles.emptyDropZone()}>Period {period}</div>
      )}
    </div>
  );
}
