...
export function useScheduleDisplayData(schoolId: string, date: string) {
  ...
  const staff = useNYCPSStaffList({ filters: { schoolIds: [schoolId] }, limit: 100 });
  ...
  const rawError = school.error || staff.error || bellSchedules.error || teacherSchedules.error;
  const error = rawError ? handleClientError(
    rawError, 
