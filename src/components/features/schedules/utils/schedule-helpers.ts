...
export const getTeacherName = (
  ...
  return (
    teachers.find((t) => t._id === teacherId)?.staffName || "Unknown Teacher"
  );
};
...
export const getBlocksForTeacherPeriod = (
  ...
  return visitBlocks.filter(
    (block) =>
      block.periodNumber === period && block.staffIds?.includes(teacherId),
  );
};

export const hasBlockInPeriod = (
  visitBlocks: VisitScheduleBlock[],
  period: number,
): boolean => {
 ...
 */
export const getTeacherPlanningStatus = (
  visitBlocks: VisitScheduleBlock[],
  teacherId: string,
) => {
...
export const isPeriodPortionAvailable = (
  ...
  if (portion === "full_period") {
    const hasAnyPortion = visitBlocks.some(
      (block) =>
        block.periodNumber === period && block.staffIds?.includes(teacherId),
    );
 ...
 */
export const getTotalBlocksCount = (
  visitBlocks: VisitScheduleBlock[],
): number => {
