...
import { useScheduleDisplayData, useScheduleUI, useScheduleComponentsActions } from "./hooks";
...
export function ScheduleBuilderContainer({ schoolId, date }: { schoolId: string, date: string }) {
  return (
    <ErrorBoundary
      context={createScheduleComponentsDataErrorContext('render', schoolId, date)}
      variant="default"
...
function ScheduleBuilderContent({ schoolId, date }: { schoolId: string, date: string }) {
