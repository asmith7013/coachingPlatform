...
export function ScoreEntry({ teacherId, cycleId, lookFor }: ScoreEntryProps) {
  ...
  const rubricLabels = lookFor.levels.map((level) => ({
    ...
    hex: level.value === 1 ? '#f8b89c' :
         level.value === 2 ? '#ffe599' :
         level.value === 3 ? '#b6d7a8' :
         '#274e13',
  }));
    ...
    <div className={cn(
      ...
      <div className="flex gap-4">
        ...
        <div className={cn(
          ...
          <Text textSize="2xl" weight="bold">#{lookFor.id}</Text>
        </div>
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <Text textSize="2xl" weight="bold">{lookFor.name}</Text>
          </div>
          <div className="mt-2">
            {lookFor.description}
          </div>
      ...
      <div className={cn(
        ...
        <Slider
          ...
          step={0.1}
          value={score}
          onChange={(value: number | number[]) => setScore(Array.isArray(value) ? value[0] : value)}
