/**
 * Shared utilities for working with standards across scope-and-sequence pages
 */
...
/**
 * Extract grade number from a scope sequence tag
 * Examples: "IM-7" -> "7", "Grade 8" -> "8", "IM-8" -> "8"
 */
export function extractGradeFromTag(tag: string): string | null {
  // Try to match various formats
  const match = tag.match(/(?:IM-|Grade\s*)(\d+)/i);
