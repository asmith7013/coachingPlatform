...
export interface StateTestQuestion {
  questionId: string;
  standard: string;
...
export function StandardsUnitMatrix({
  ...
  const setShowSubstandards =
    onShowSubstandardsChange ?? setShowSubstandardsLocal;
  ...
  const matrixData = useMemo(() => {
    ...
    questions.forEach((q) => {
      ...
      if (primaryStd.startsWith(`${selectedGrade}.`)) {
        standardQuestionCounts.set(
          primaryStd,
          (standardQuestionCounts.get(primaryStd) || 0) + 1,
        );
      }
      ...
      if (q.secondaryStandard) {
        ...
        if (secondaryStd.startsWith(`${selectedGrade}.`)) {
          standardQuestionCounts.set(
            secondaryStd,
            (standardQuestionCounts.get(secondaryStd) || 0) + 1,
          );
        }
      }
    });

    const totalQuestions = Array.from(standardQuestionCounts.values()).reduce(
      (sum, count) => sum + count,
      0,
    );
    ...
    standardQuestionCounts.forEach((count, std) => {
      ...
      units.forEach((unit) => {
        ...
        const hasStandard = unitStds.some((unitStd) => {
          ...
          const processedUnitStd = showSubstandards
            ? normalizedUnitStd
            : stripLetterSuffix(normalizedUnitStd);
          ...
          return (
            processedUnitStd === std || standardMatches(processedUnitStd, std)
          );
        });
    ...
    byDomain.forEach((rows) => {
      rows.sort((a, b) => {
        ...
        const aParentNum = parseInt(
          a.parentStandard.split(".").pop() || "0",
          10,
        );
        const bParentNum = parseInt(
          b.parentStandard.split(".").pop() || "0",
          10,
        );
    ...
    const domainGroups: DomainGroup[] = Array.from(byDomain.entries())
      .map(([domain, standards]) => {
        const totalDomainQuestions = standards.reduce(
        ...
        return {
          ...
          totalPercent:
            totalQuestions > 0
              ? Math.round((totalDomainQuestions / totalQuestions) * 100)
              : 0,
        };
      })
    ...
    sortedUnits.forEach((unit) => {
      ...
      const coveragePercent =
        totalQuestions > 0
          ? Math.round((questionSum / totalQuestions) * 100)
          : 0;
      unitCoverage.set(unit.unitNumber, {
    ...
    const untaggedCount = untaggedStandards.reduce(
      (sum, row) => sum + row.questionCount,
      0,
    );
    const untaggedPercent =
      totalQuestions > 0
        ? Math.round((untaggedCount / totalQuestions) * 100)
        : 0;
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <p className="text-gray-400 text-sm">
          Select a grade to view the standards matrix.
        </p>
      </div>
    );
  }
  ...
  const maxPercent = Math.max(
    ...domainGroups.flatMap((g) => g.standards.map((s) => s.percent)),
  );
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Shows how many state test questions align with each standard taught in
        each unit.
      </p>
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 z-20">
            ...
            <tr className="border-b border-gray-200">
              <th
                rowSpan={2}
                className="text-left py-1.5 px-2 font-semibold text-gray-700 bg-gray-50 sticky left-0 z-30 min-w-[80px] border-b-2 border-gray-300"
              >
              ...
              <th
                rowSpan={2}
                className="text-center py-1.5 px-1.5 font-semibold text-gray-700 bg-gray-50 min-w-[45px] border-b-2 border-gray-300"
              >
                Test %
              </th>
              <th
                rowSpan={2}
                className="text-center py-1.5 px-1.5 font-semibold text-gray-700 bg-gray-50 min-w-[40px] border-b-2 border-gray-300"
              >
                # Qs
              </th>
              ...
              {domainGroups.map((group) => {
                ...
                const domainPercentColorClass = getPercentColor(
                  group.totalPercent,
                );
                return (
                  <th
                  ...
                  >
                    <div className="flex flex-col leading-tight">
                      ...
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span
                          className={`text-[12px] px-1 rounded ${domainPercentColorClass}`}
                        >
                          {group.totalPercent}%
                        </span>
                      </div>
            ...
            <tr className="border-b-2 border-gray-300">
              ...
              {domainGroups.flatMap((group) =>
                group.standards.map((row, idx) => {
                  ...
                  const description =
                    standardDescriptions[row.standard] ||
                    standardDescriptions[row.parentStandard] ||
                    STANDARD_DESCRIPTIONS[row.standard] ||
                    STANDARD_DESCRIPTIONS[row.parentStandard];
                  ...
                  const isNewParentGroup =
                    ...
                    idx > 0 &&
                    prevRow &&
                    prevRow.parentStandard !== row.parentStandard;
                  return (
                    <th
                      key={row.standard}
                    ...
                    >
                      ...
                      >
                        <div className="cursor-help flex flex-col items-center leading-tight">
                          ...
                          <span
                            className={`text-[10px] px-1 rounded ${percentColorClass}`}
                          >
                            {row.percent}%
                          </span>
                        </div>
          ...
          <tbody>
            {sortedUnits.map((unit, unitIndex) => {
              const coverage = unitCoverage.get(unit.unitNumber) || {
                percent: 0,
                count: 0,
              };
              ...
              return (
                <tr
                  key={unit.unitNumber}
                  onClick={() =>
                    onUnitClick?.(isSelected ? null : unit.unitNumber)
                  }
                  className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${
                    unitIndex === sortedUnits.length - 1
                ...
                >
                  ...
                  <td className="py-1 px-2 sticky left-0 z-10 bg-white">
                    <div className="flex flex-col leading-tight">
                      ...
                      <span
                        className={`text-[11px] font-medium truncate max-w-[100px] ${isSelected ? "text-blue-700" : "text-gray-700"}`}
                      >
                        {unit.unitName?.replace(/^Unit\s*\d+\s*[-–]\s*/i, "") ||
                          `Unit ${unit.unitNumber}`}
                      </span>
                      {/* Abbreviation - lighter */}
                      <span
                        className={`text-xs ${isSelected ? "text-blue-500" : "text-gray-500"}`}
                      >
                        U{unit.unitNumber}
                      </span>
                    </div>
                  ...
                  {domainGroups.flatMap((group) =>
                    group.standards.map((row, idx) => {
                      const isPresent =
                        row.unitPresence.get(unit.unitNumber) || false;
                      const colors =
                        DOMAIN_COLORS[group.domain] || DOMAIN_COLORS.Other;
                      // Check if this is first of a new parent standard group (when showing substandards)
                      const prevRow = idx > 0 ? group.standards[idx - 1] : null;
            ...
            {untaggedStandards.length > 0 && (
              <tr className="border-t-2 border-gray-300 bg-gray-50">
                ...
                {domainGroups.flatMap((group) =>
                  group.standards.map((row, idx) => {
                    const isUntagged = untaggedStandards.some(
                      (u) => u.standard === row.standard,
                    );
                    const colors =
                      DOMAIN_COLORS[group.domain] || DOMAIN_COLORS.Other;
                    // Check if this is first of a new parent standard group (when showing substandards)
                    ...
                    return (
                      ...
                      >
                        {isUntagged ? (
                          ...
                          <span className="inline-flex items-center justify-center w-5 h-5 text-gray-200 text-[10px]"></span>
                        )}
                      </td>
                    );
                  }),
      ...
      <Legend title="Legend">
        <LegendGroup>
          <LegendItem
            icon={
              <span className="inline-flex items-center justify-center w-5 h-5 bg-blue-500 text-white text-[10px] font-bold rounded">
                5
              </span>
            }
            label={
              <span className="text-xs text-gray-600">
                # questions on state test
              </span>
            }
          />
          <LegendItem
            icon={
              <span className="inline-flex items-center justify-center w-5 h-5 border border-gray-200 text-gray-300 rounded text-[10px]">
                —
              </span>
            }
