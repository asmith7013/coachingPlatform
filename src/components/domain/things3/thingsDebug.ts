...
export function validateThingsJson(json: unknown): ValidationResult {
  ...
  try {
    ...
    thingsJson.items.forEach((item: unknown, index: number) => {
      ...
      if (
        !projectItem.attributes ||
        typeof projectItem.attributes !== "object"
      ) {
      ...
      if (!Array.isArray(projectItem.data)) {
        errors.push(
          `Project ${index} must have a "data" array (not "items" or "children")`,
        );
      } else {
        // 3.4 Verify each item in the data array
        (projectItem.data as unknown[]).forEach(
          (dataItem: unknown, dataIndex: number) => {
            if (typeof dataItem !== "object" || dataItem === null) {
              errors.push(
                `Project ${index}, data item ${dataIndex} must be an object`,
              );
              return;
            }
            ...
            if (thingsItem.type !== "heading" && thingsItem.type !== "to-do") {
              errors.push(
                `Project ${index}, data item ${dataIndex} has invalid type "${thingsItem.type}". Must be "heading" or "to-do"`,
              );
            }

            if (
              !thingsItem.attributes ||
              typeof thingsItem.attributes !== "object"
            ) {
              errors.push(
                `Project ${index}, data item ${dataIndex} must have attributes with a title`,
              );
            } else {
              const attributes = thingsItem.attributes as Record<
                string,
                unknown
              >;
              if (!attributes.title) {
                errors.push(
                  `Project ${index}, data item ${dataIndex} must have a title`,
                );
              }
            }
          },
    ...
    items: [
      {
        ...
        data: [
          // IMPORTANT: Use "data" array, not "items" or "children"
          {
 ...
 */
export function convertChecklistToThingsData(
  checklist: ThingsChecklist,
): ThingsData {
...
export function troubleshootThingsUrl(jsonData: unknown): TroubleshootResult {
  ...
  if (Array.isArray(dataRecord.items) && dataRecord.items.length > 0) {
    ...
    if (firstItem.type === "heading" || firstItem.type === "task") {
      parsedData = convertChecklistToThingsData(
        originalData as ThingsChecklist,
      );
    } else {
  ...
  try {
    ...
    generatedUrl =
      "Error generating URL from original data: " + (error as Error).message;
  }
