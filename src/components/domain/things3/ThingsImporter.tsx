...
export default function ThingsImporter() {
  ...
  const processChecklist = () => {
    try {
      ...
      if (inputMode === 'json') {
        // Parse JSON input
        if (!jsonInput.trim()) {
          showMessage("Missing Content", "Please enter JSON data first!", "error");
          return;
        }
        ...
        try {
          ...
          showMessage("Invalid JSON", "The JSON data is not valid. Please check the format.", "error");
          console.error(err);
          return;
        }
      } else {
        ...
        if (!checklistInput.trim()) {
          showMessage("Missing Content", "Please paste your checklist first!", "error");
          return;
        }
        
        checklist = parseTextToJSON(checklistInput, projectTitle, projectNotes, projectWhen);
      }
      ...
      showMessage("URL Generated", "Your Things 3 import URL has been created", "success");
    } catch (error) {
      showMessage("Error", "Failed to process checklist: " + (error as Error).message, "error");
  ...
  return (
    <div className="container mx-auto py-6">
      <PageHeader title="Things 3 Importer" subtitle="Create Things 3 projects from structured data" />
      ...
      <Card>
        ...
        <Text>Convert your checklist into a Things 3 project with sections and tasks.</Text>
        ...
        <div className="flex space-x-4 mt-6">
          <Button onClick={processChecklist}>
            Generate URL
          </Button>
          <Button onClick={openInThings} intent="secondary">
            Open in Things 3
          </Button>
          <Button onClick={copyToClipboard} intent="secondary" appearance="outline">
            Copy URL
          </Button>
        </div>
      </Card>
