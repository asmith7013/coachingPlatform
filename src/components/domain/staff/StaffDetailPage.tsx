...
export function StaffDetailPage({ staffType = "nycps" }: StaffDetailPageProps) {
  ...
  const {
    ...
    schema: (staffType === "nycps"
      ? NYCPSStaffZodSchema
      : TeachingLabStaffZodSchema) as ZodSchema<StaffMember>,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Tabs tabs={tabs} defaultTab="info" textSize="base" />
    </div>
  );
}
  ...
  if (error) {
    return (
      <Card className="p-4 mt-4">
        <Text color="danger">
          Failed to load staff information: {error.message}
        </Text>
      </Card>
    );
  }
          ...
          <InfoItem
            ...
            value={
              staffMember.createdAt
                ? formatMediumDate(
                    toDateString(new Date(staffMember.createdAt)),
                  )
                : "Unknown"
            }
          />
          <InfoItem
            ...
            value={
              staffMember.updatedAt
                ? formatMediumDate(
                    toDateString(new Date(staffMember.updatedAt)),
                  )
                : "Unknown"
            }
          />
        ...
        <Card className="p-6">
          ...
          <dl className="grid grid-cols-1 gap-x-4 gap-y-2">
            ...
            <InfoItem
              ...
              value={
                staffMember.assignedDistricts &&
                staffMember.assignedDistricts.length > 0
                  ? `${staffMember.assignedDistricts.length} districts`
                  : "None"
              }
            />
          </dl>
...
function InfoItem({ label, value }: { label: string; value: string }) {
  return (
    <>
      <dt className={cn("text-sm", weight.medium, "text-muted")}>{label}</dt>
    ...
    () => [
      {
        ...
        action: "Created",
        user: "System Admin",
        details: "Initial staff record created",
      },
      {
        date: "2023-12-18",
        action: "Updated",
        user: "John Doe",
        details: "Schedule information added",
      },
      {
        date: "2024-01-10",
        action: "Updated",
