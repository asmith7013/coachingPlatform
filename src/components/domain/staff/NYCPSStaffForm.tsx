...
export function NYCPSStaffForm({
  ...
  const gradeLevelOptions = GradeLevelsSupportedZod.options.map(
    (value: string) => ({ value, label: value }),
  );
  const roleOptions = RolesNYCPSZod.options.map((value: string) => ({
    value,
    label: value,
  }));
        ...
        <form.Field name="specialGroups">
          {(field) => (
            <Input
              ...
              placeholder="Enter special groups (comma separated)"
            />
          )}
        </form.Field>
