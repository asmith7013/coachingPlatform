...
export function SchoolDetailView({ schoolId }: SchoolDetailViewProps) {
  ...
  const schoolStaff = useMemo(() => {
    ...
    return allStaff.filter(
      (staff) => staff.schoolIds && staff.schoolIds.includes(schoolId),
    );
  }, [allStaff, schoolId]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        ...
        <Button className="bg-red-500 text-white hover:bg-red-600" padding="md">
          Delete
        </Button>
      </div>
      ...
      <Card padding="lg" radius="lg">
        <Heading
          level="h2"
          className="text-xl font-semibold bg-blue-500 text-white px-4 py-2 rounded mb-6"
        >
          Grade Levels
        </Heading>
        <div className="flex flex-wrap gap-2">
          {school.gradeLevelsSupported &&
            school.gradeLevelsSupported.map((grade: string, index: number) => (
              <span
                key={index}
                className={cn(
      ...
      <Card padding="lg" radius="lg">
        <div className="flex justify-between items-center mb-6">
          <Heading
            level="h2"
            className="text-xl font-semibold bg-blue-500 text-white px-4 py-2 rounded"
          >
            Visits
          </Heading>
        ...
        {staffLoading ? (
          ...
          >
            {schoolStaff.map((teacher, index) => {
              ...
              const primaryRole =
                teacher.rolesNYCPS && teacher.rolesNYCPS.length > 0
