 ...
 */
export function StaffListDebugger({
  children,
  mode = "full",
}: StaffListDebuggerProps) {
  ...
  useEffect(() => {
    renderCount.current++;
    console.log(
      `StaffListDebugger rendered (${renderCount.current}), mode: ${mode}`,
    );
  ...
  return (
    <IsolateComponent title={`StaffList Debug (${mode} mode)`}>
      <RenderLoopDetector componentId={`staff-list-${mode}`}>
        <SWRDebugger componentId={`staff-swr-${mode}`}>{children}</SWRDebugger>
      </RenderLoopDetector>
      ...
      <div className="mt-4 p-3 bg-gray-800 text-white rounded text-xs">
        ...
        <ul className="list-disc pl-4 mt-2 space-y-1">
          <li>
            Current bisection mode: <strong>{mode}</strong>
          </li>
    ...
    useEffect(() => {
      ...
      if (propsRef.current) {
        ...
        if (unstableObjects.length > 0) {
          console.warn(
            "⚠️ Unstable objects/functions detected:",
            unstableObjects,
          );
