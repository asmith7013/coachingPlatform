...
export function useUserStaff() {
  ...
  const hasValidStaffId = metadata.staffId
    ? isValidObjectId(metadata.staffId)
    : false;
  ...
  const enabled =
    isSignedIn &&
    Boolean(metadata.staffId) &&
    Boolean(metadata.staffType) &&
    hasValidStaffId;

  // Determine the correct API endpoint based on staff type
  const apiEndpoint = (() => {
    if (!metadata.staffId || !metadata.staffType || !hasValidStaffId)
      return null;
    return `/api/staff/${metadata.staffId}?type=${metadata.staffType}`;
  })();

  // Query key includes staff type for proper cache management
  const queryKey = ["staff", "current", metadata.staffId, metadata.staffType];
    ...
    queryFn: async () => {
      try {
        ...
        if (response.status === 404) {
          console.warn(
            `Staff member not found: ${metadata.staffId}. This may indicate outdated user metadata.`,
          );
          return null;
        }
