...
function useVisitManagerWithScheduling() {
   ...
   */
  const createMultipleVisits = useCallback(
    async (visitsData: DocumentInput<Visit>[]) => {
      try {
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : "Failed to create visits",
        };
      }
    },
    [manager],
   ...
   */
  const getVisitsForSchoolAndDate = useCallback(
    (schoolId: string, date: Date) => {
