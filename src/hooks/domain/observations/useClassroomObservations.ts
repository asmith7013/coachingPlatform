...
const useClassroomObservationsMutations =
  classroomObservationHooks.useMutations;
      ...
      return notifications.withToast(
        () =>
          mutations.updateAsync!(
            id,
            data as DocumentInput<ClassroomObservation>,
          ),
        toastConfig.update!,
        enableToasts,
      );
...
function useClassroomObservationsWithAutoSave() {
  const createInitialDraft = useCallback(
    async (
      partialData: Partial<ClassroomObservationInput> = {},
    ): Promise<string> => {
      ...
      const completeData: ClassroomObservationInput =
        ClassroomObservationInputZodSchema.parse({
          ...defaults,
          ...partialData,
          status: "draft",
        });
      const result = await createClassroomObservation(
        completeData as ClassroomObservationInput,
      );
  ...
  const createAutoSaveConfig = useCallback(
    (observationId: string, formData: unknown) => {
      return {
        entityId: observationId,
        data: formData,
        onSave: async (id: string, data: unknown) => {
          const result = await autoSaveObservationData(
            id,
