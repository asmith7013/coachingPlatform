...
export function useComponentTester({
  ...
  const [wrapperType, setWrapperType] = useState<DebugWrapperType>(initialWrapperType);
  
  // Function to wrap the component based on selected debug mode
  const wrappedComponent = useCallback((component: React.ReactNode) => {
    ...
    const wrappedWithDebugger = (() => {
      switch (wrapperType) {
        case 'render-loop':
          return <RenderLoopDetector componentId={componentId}>{component}</RenderLoopDetector>;
        case 'swr':
          return <SWRDebugger componentId={componentId}>{component}</SWRDebugger>;
        case 'both':
          return (
  ...
  const toggleDebugMode = useCallback(() => {
    setWrapperType(current => {
      switch (current) {
        case 'none': return 'render-loop';
        case 'render-loop': return 'swr';
        case 'swr': return 'both';
