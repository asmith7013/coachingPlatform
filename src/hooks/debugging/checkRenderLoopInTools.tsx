...
export function useCheckForLoopsInDebugTools() {
  ...
  useEffect(() => {
    ...
    if (renderTimes.current.length > 1) {
      ...
      timeSinceLastRender = renderTimes.current[lastIndex] - renderTimes.current[lastIndex - 1];
    }
  ...
  return (
    <div className="p-4 bg-gray-100 rounded border mb-4">
      ...
      <div className="mt-2 text-sm">
        <p>Render count: <span className="font-mono">{stats.count}</span></p>
        <p>Last interval: <span className="font-mono">{stats.interval}ms</span></p>
        <p>Fast renders: <span className="font-mono">{stats.fastRenders}</span></p>
        
        <div className={`mt-2 p-2 rounded ${isLooping ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
          {isLooping 
        ...
        {isLooping && (
          <ul className="mt-2 list-disc pl-5 text-sm">
            <li>If you see this message, your debugging components might be causing the render loop</li>
            <li>Try the <code>/debug/no-debug-tools</code> page to confirm</li>
            <li>Check the dependencies in your debugging tools&apos; useEffect hooks</li>
          </ul>
        )}
      </div>
    </div>
