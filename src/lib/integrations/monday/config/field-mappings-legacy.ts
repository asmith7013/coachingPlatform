...
export const OPTIONAL_FIELD_MAPPERS: FieldMappers = {
  ...
  "grade_levels_column": {
    ...
    transform: (value: string): VisitInput["gradeLevelsSupported"] => {
      ...
      const grades = String(value).split(",").map(g => g.trim());
 ...
 */
export function getMappingByTitle(title: string): FieldMapper<keyof VisitInput> | undefined {
  ...
  for (const column of columns) {
    ...
    switch (column.type) {
      ...
      case "text":
        ...
        if (column.title.toLowerCase().includes("school") && 
