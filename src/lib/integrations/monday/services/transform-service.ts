 ...
 */
export async function transformMondayItemToVisit(
  ...
  for (const [mondayField, mapping] of Object.entries(FIELD_MAPPERS)) {
    try {
      ...
      const columnValue = mondayItem.column_values.find(
        (col: MondayColumnValue) =>
          col.id === mondayField || col.title === mondayField,
      );
  ...
  if (
    ...
    const coachId =
      typeof transformed.coach === "string"
        ? transformed.coach
  ...
  return {
    ...
    success:
      validForImport &&
      requiredForFinalValidation.length === 0 &&
 ...
 */
export function transformVisitToMondayItem(
  visit: Visit,
  ...
  const reverseTransformers: Record<
    string,
  ...
  for (const [fieldName, fieldValue] of Object.entries(visit)) {
    ...
    if (
      ...
      fieldName === "createdAt" ||
      fieldName === "updatedAt" ||
      fieldName.startsWith("monday")
    ) {
      // Skip monday-specific fields
      continue;
    }
