...
export function dynamicTransformMondayItemToVisit(
  ...
  return mapMondayColumnsToVisitFields(mondayItem, boardColumns);
}
...
export function enhancedTransformMondayItemToVisit(
  ...
  const { visitData, missingFields } = mapMondayColumnsToVisitFields(mondayItem, boardColumns);
  
  // Determine overall validity
  const valid = missingFields.length === 0;
  
  return {
    transformed: visitData,
    valid,
    success: valid,
