...
export class SchemaTransformer<T extends Record<string, unknown>> {
  ...
  transformMondayItemToEntity(
    ...
    const { entityData, missingFields } = this.mapMondayColumnsToEntityFields(
      mondayItem,
      boardColumns,
    );
   ...
   */
  protected postProcessEntityData() // entityData: Partial<T>
  : void {
  ...
  protected handleFallbackMapping(
    ...
    Object.entries(valuesById).forEach(([colId, value]) => {
      ...
      for (const [pattern, fieldName] of Object.entries(
        this.config.idPatternMappings,
      )) {
      ...
      if (
        matchedField &&
        this.isFieldMissing(entityData, matchedField, missingFields)
      ) {
