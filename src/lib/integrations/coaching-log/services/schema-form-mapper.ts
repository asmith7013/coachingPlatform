...
} from "@/lib/ui/constants/coaching-log";
...
export class SchemaFormMapper {
  ...
  private getEnhancedDefaults(
    ...
    const smartBooleanDefaults = {
      oneOnOneCoachingDone: getSmartBooleanDefault(
        "oneOnOneCoachingDone",
        coachingLog,
      ),
      microPLDone: getSmartBooleanDefault("microPLDone", coachingLog),
      modelingPlanningDone: getSmartBooleanDefault(
  ...
  private mapEnumFields(data: CoachingLogInput): Record<string, string> {
    return {
      ...
      totalDuration:
        (data.totalDuration as string) ||
        EnumFieldMappings.totalDuration.options[0],
      Solvestouchpoint: this.mapSolvesTouchpointValue(
        (data.solvesTouchpoint as string) ||
          EnumFieldMappings.solvesTouchpoint.options[0],
      ),

      // NEW: Admin fields with smart defaults
      NYCSolvesAdmin:
        (data.NYCSolvesAdmin as string) ||
        this.determineNYCSolvesAdminDefault(data),
      adminDone:
        (data.adminDone as string) || this.determineAdminDoneDefault(data),
   ...
   */
  private mapNumericFields(
    data: CoachingLogInput,
    overrides: FormOverrides,
   ...
   */
  private mapSpecialFields(
    data: CoachingLogInput,
    overrides: FormOverrides,
  ): Record<string, string | string[]> {
    return {
      NYCDone: NYCCoachMapping.transformToForm(
        (data.NYCDone as boolean) || false,
      ),
      ...
      teachersSupportedType: (data.teachersSupportedTypes as string[])?.length
  ...
  private getGradeLevelsForForm(
    ...
    const gradeLevels = overrides.schoolGradeLevels?.length
      ? overrides.schoolGradeLevels
      : (data.gradeLevelsSupported as string[])?.length
        ? (data.gradeLevelsSupported as string[])
  ...
  private mapOverrideFields(overrides: FormOverrides): Record<string, string> {
    return {
      ...
      coachingDate:
        overrides.visitDate || new Date().toISOString().split("T")[0],
  ...
  private getImplementationIndicator(data: CoachingLogInput): string {
    const experience =
      (data.implementationExperience as string) ||
      "First year of Implementation";
    const options = getImplementationOptions(experience);
    return (
      (data.implementationFocus as string) ||
      options[0] ||
  ...
  private getPrimaryStrategyCategory(data: CoachingLogInput): string {
    return (
      (data.primaryStrategyCategory as string) ||
      (data.primaryStrategy as string) ||
      "In-class support"
    );
  }
  ...
  private getSolvesSpecificStrategy(data: CoachingLogInput): string {
    ...
    return (
      (data.primaryStrategySpecific as string) ||
      (data.solvesSpecificStrategy as string) ||
      options[0] ||
  ...
  private determineAdminDoneDefault(data: CoachingLogInput): string {
    ...
    return (data.solvesTouchpoint as string[])?.includes("Leader support")
      ? "Yes"
      : "No";
  }
}
