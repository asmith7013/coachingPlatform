...
export class SpreadsheetProcessor {
  ...
  public processRow(
    ...
    try {
      ...
      const validatedDaily = validateArraySafe(DailyClassEventInputZodSchema, [
        dailyEvent,
      ]);
      const validatedZearn = validateArraySafe(
        ZearnCompletionInputZodSchema,
        zearnCompletions,
      );
      const validatedSnorkl = validateArraySafe(
        AssessmentCompletionInputZodSchema,
        snorklCompletions,
      );
      ...
      return createCollectionResponse(
        [],
        handleServerError(error, "SpreadsheetRowProcessing"),
      );
    }
  }
