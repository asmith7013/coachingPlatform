...
export class SpreadsheetColumnMapper {
  ...
  public validateHeaders(): { success: boolean; missingColumns: string[] } {
    ...
    const missing = required.filter(
      (col) => this.columnIndexes[col] === undefined,
    );
...
export class SpreadsheetRowValidator {
  ...
  public validateRow(
    row: RawSpreadsheetRow,
    headers: SpreadsheetHeaders,
  ): ValidatedRowData | null {
    try {
      const getValue = (col: string) =>
        this.mapper.extractValue(row, headers, col);
      ...
      const attendance = validateStrict(
        AttendanceStatusZod,
        getValue("attendance"),
      );
      ...
      if (isNaN(studentId))
        throw new Error(`Invalid Student ID: ${getValue("studentId")}`);
