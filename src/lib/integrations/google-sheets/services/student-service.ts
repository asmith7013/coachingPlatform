...
export class StudentService {
  ...
  static async fetchStudentsBySection(section: string) {
    try {
      const students = await StudentModel.find({ section, active: true }).sort({
        lastName: 1,
        firstName: 1,
      });
  ...
  static async fetchStudentsByTeacher(teacher: string) {
    try {
      const students = await StudentModel.find({ teacher, active: true }).sort({
        section: 1,
        lastName: 1,
        firstName: 1,
      });
  ...
  static async bulkCreateStudents(studentsData: StudentInput[]) {
    try {
      console.log(
        "[StudentService] bulkCreateStudents called with",
        studentsData.length,
        "students",
      );
      ...
      console.log(
        "[StudentService] All students validated, calling insertMany...",
      );
      ...
      console.log(
        "[StudentService] Found",
        existingStudents.length,
        "existing students with these IDs",
      );
      if (existingStudents.length > 0) {
        console.log(
          "[StudentService] Existing student IDs:",
      ...
      console.log(
        "[StudentService] insertMany rawResult:",
      ...
      const verifyCount = await StudentModel.countDocuments({
        studentID: { $in: studentIDs },
      });
      console.log(
        "[StudentService] Verification: Found",
        verifyCount,
        "students in DB after insert",
      );
      ...
      const insertedStudents = await StudentModel.find({
        studentID: { $in: studentIDs },
      });
      ...
      if (error && typeof error === "object" && "insertedDocs" in error) {
        ...
        console.log(
          "[StudentService] Partial success:",
          insertedCount,
          "students created",
        );
        ...
        return {
          ...
          warning:
            "Some students may have been skipped due to duplicate student IDs",
        };
      }
