import { getSheetMetadata, fetchSheetData, appendSheetData } from "../client";
...
export class GoogleSheetsExportService {
  ...
  async exportAndResetDailyData(
    ...
    try {
      ...
      if (dailySheets.length === 0) {
        throw new Error(
          `No sheets found starting with "${GoogleSheetsExportService.DAILY_SHEET_PREFIX}"`,
        );
      }
  ...
  private async findDailySheets() {
    ...
    return (
      metadata.data.sheets?.filter((sheet) =>
        sheet.properties?.title?.startsWith(
          GoogleSheetsExportService.DAILY_SHEET_PREFIX,
        ),
      ) || []
    );
  }
  ...
  private async processSheet(
    ...
    const sheetTitle =
      (sheet as { properties?: { title?: string } })?.properties?.title ||
      "Unknown Sheet";

    // Step 1: Export to Combined Data
    const exportResult = await this.exportToCombinedData(
      sheetTitle,
      exportDate,
    );
  ...
  private async exportToCombinedData(sheetTitle: string, exportDate: Date) {
    ...
    if (!dataResult.success) {
      throw new Error(
        `Failed to read data from ${sheetTitle}: ${dataResult.error}`,
      );
    }
  ...
  private async checkForDuplicates(
    newDataRows: (string | number | boolean)[][],
    exportDate: Date,
    sheetName: string,
  ) {
    ...
    newDataRows.forEach((newRow) => {
      ...
      existingResult.data.forEach((existingRow) => {
        ...
        const existingExportDate =
          existingRow[existingRow.length - 2]?.toString() || ""; // Export Date column
        const existingSheetName =
          existingRow[existingRow.length - 1]?.toString() || "Unknown Sheet"; // Last column is sheet name

        if (
          existingExportDate &&
          new Date(existingExportDate).toLocaleDateString() === exportDateStr
        ) {
          if (
  ...
  private async resetSheetForNextDay(sheetTitle: string) {
    try {
      ...
      const result = await this.resetService.resetSheetForNextDay(
        worksheetId,
        sheetTitle,
      );
      return result;
