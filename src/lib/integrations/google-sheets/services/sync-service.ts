...
} from "@/lib/schema/mongoose-schema/scm/core";
import {
...
import {
  ...
  validateHeaders,
} from "@/lib/integrations/google-sheets/data-processor";
...
export class GoogleSheetsSyncService {
  ...
  static async syncSheetsData(
    ...
    try {
      ...
      console.log(
        `⚡ Processing result: ${result.metadata.successfulRows} successful, ${result.metadata.failedRows} failed`,
      );
      console.log(
        `📈 Generated ${result.data.dailyEvents.length} daily events and ${result.data.lessonCompletions.length} lesson completions`,
      );
      ...
      if (result.data.dailyEvents.length > 0) {
        try {
          ...
          const dailyEventResult = await DailyClassEventModel.insertMany(
            validatedDailyEvents,
            {
              ordered: false,
            },
          );
          ...
          console.log(
            `✅ Successfully inserted ${insertedDailyEvents} daily events`,
          );
        } catch (error) {
          if (error && typeof error === "object" && "insertedDocs" in error) {
            const insertedDocs = (
              error as unknown as { insertedDocs: unknown[] }
            ).insertedDocs;
            insertedDailyEvents = insertedDocs?.length || 0;
            console.log(
              `⚠️ Partial success: inserted ${insertedDailyEvents} daily events with some duplicates`,
            );
          } else {
      ...
      if (result.data.lessonCompletions.length > 0) {
        try {
          const validatedCompletions = result.data.lessonCompletions.map(
            (completion) => {
          ...
          const completionResult = await LessonCompletionModel.insertMany(
          ...
          console.log(
            `✅ Successfully inserted ${insertedLessonCompletions} lesson completions`,
          );
        } catch (error) {
          if (error && typeof error === "object" && "insertedDocs" in error) {
            const insertedDocs = (
            ...
            console.log(
              `⚠️ Partial success: inserted ${insertedLessonCompletions} lesson completions with some duplicates`,
            );
          } else {
      ...
      return {
        ...
        error:
          insertErrors.length > 0
            ? `Insert errors: ${insertErrors.map((e) => e.error).join(", ")}`
            : undefined,
      };
    } catch (error) {
  ...
  static async getSyncStatus() {
    try {
      ...
      const lessonCompletionsCount =
        await LessonCompletionModel.countDocuments();

      // Get latest records to determine last sync
      const latestDailyEvent = await DailyClassEventModel.findOne().sort({
        updatedAt: -1,
      });
      const latestCompletion = await LessonCompletionModel.findOne().sort({
        updatedAt: -1,
      });

      const lastSyncDate =
        latestDailyEvent?.updatedAt > latestCompletion?.updatedAt
