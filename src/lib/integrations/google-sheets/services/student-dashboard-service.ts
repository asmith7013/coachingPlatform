import {
  ...
  StudentZearnProgress,
} from "@zod-schema/scm/student/student-data";
...
export class StudentDashboardService {
  ...
  private static readonly STUDENT_DATA_SPREADSHEET_ID =
    process.env.GOOGLE_SHEETS_STUDENT_DATA_ID || "";
   ...
   */
  static async fetchStudentDataFromSheets(
    studentId: string,
   ...
   */
  static async validateStudentEmail(
    email: string,
    studentId: string,
    ...
    try {
      // Find student by ID
      const student = await StudentModel.findOne({
        $or: [{ _id: studentId }, { studentID: parseInt(studentId) }],
        active: true,
      }).lean();
      ...
      const normalizedStudentEmail = (student.email as unknown as string)
        .toLowerCase()
        .trim();
      ...
      const similarity = this.calculateEmailSimilarity(
  ...
  private static parseStudentDataFromSheet(
    ...
    try {
      ...
      const studentRows = rows
        .slice(1)
        .filter((row) => row[this.COLUMN_MAPPING.STUDENT_ID] === studentId);
   ...
   */
  private static determineStudentGrade(
    zearnProgress: StudentZearnProgress[],
    section: string,
  ): GradeLevel {
   ...
   * Parse Zearn progress from Zearn lessons column (Column O)
   */
  private static parseZearnProgress(
    studentRows: string[][],
  ): StudentZearnProgress[] {
    ...
    studentRows.forEach((row) => {
      ...
      if (zearnLessonsText && date) {
        // Parse comma-separated lessons: "G6 U2 L05, G6 U2 L03, G6 U2 L06"
        const lessons = zearnLessonsText
          .split(",")
          .map((l) => l.trim())
          .filter((l) => l);
   ...
   */
  private static calculateWeeklyMinutes(
    studentRows: string[][],
    ...
    studentRows.forEach((row, index) => {
      ...
      const lessonsCount = (row[this.COLUMN_MAPPING.ZEARN_LESSONS] || "")
        .split(",")
   ...
   */
  private static calculateEmailSimilarity(
    email1: string,
    email2: string,
  ): number {
