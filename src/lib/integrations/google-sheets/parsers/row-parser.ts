 ...
 */
export function createDailyClassEvent(
 ...
 */
export function createZearnCompletions(
  data: ValidatedRowData,
): ZearnCompletionInput[] {
  ...
  const timePerLesson =
    data.instructionReceivedMin && lessons.length > 0
 ...
 */
export function createSnorklCompletions(
  data: ValidatedRowData,
): AssessmentCompletionInput[] {
  console.log(
    `🎯 [DEBUG] Creating Snorkl completions for ${data.firstName} ${data.lastName}`,
  );
  ...
  [data.mastery1, data.mastery2, data.mastery3].forEach((mastery, index) => {
    ...
    console.log(
      `🔍 [DEBUG] Processing mastery ${masteryNum} for ${data.firstName} ${data.lastName}:`,
      mastery,
    );

    if (!mastery) {
      console.log(
        `⏭️ [DEBUG] Mastery ${masteryNum} is null/undefined, skipping`,
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        `❌ [DEBUG] Failed to create Snorkl completion ${masteryNum} for ${data.firstName} ${data.lastName}:`,
        error,
      );
  ...
  console.log(
    `📊 [DEBUG] Total Snorkl completions created for ${data.firstName} ${data.lastName}: ${completions.length}`,
  );
  return completions;
}
