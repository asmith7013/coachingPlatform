...
> {
  ...
  try {
    ...
    console.log(
      "[getUserGoogleOAuthToken] userId:",
    ...
    console.log(
      "[getUserGoogleOAuthToken] Tokens response:",
    ...
    if (!tokens || tokens.data.length === 0) {
      ...
      return {
        success: false,
        error:
          "No Google OAuth token found. Please sign in with Google and grant Drive access.",
      };
    }
    ...
    if (clerkError?.clerkError) {
      ...
      console.error(
        "[getUserGoogleOAuthToken] errors:",
        JSON.stringify(clerkError.errors, null, 2),
      );
      console.error(
        "[getUserGoogleOAuthToken] longMessage:",
    ...
    if (error instanceof Error) {
      if (error.message.includes("Unprocessable Entity")) {
        return {
          success: false,
          error:
            "Google OAuth token expired or invalid. Please sign out and sign in again with Google.",
        };
      }
...
> {
  ...
  console.log(
    "[uploadToUserDrive] convertToGoogleFormat:",
    options?.convertToGoogleFormat,
  );
  ...
  try {
    ...
    console.log(
      "[uploadToUserDrive] fileMetadata:",
    ...
    console.log(
      "[uploadToUserDrive] response.data:",
    ...
    console.log(
      "[uploadToUserDrive] SUCCESS: File uploaded, ID:",
      response.data.id,
    );
    return {
      success: true,
    ...
    console.error(
      "[uploadToUserDrive] Error message:",
      error instanceof Error ? error.message : String(error),
    );
    console.error(
      "[uploadToUserDrive] Full error:",
    ...
    console.error(
      "[uploadToUserDrive] Error keys:",
      Object.keys(googleError || {}),
    );
    ...
    const responseStatus =
      googleError?.response?.status || googleError?.status || googleError?.code;
    ...
    if (errorsArray) {
      console.error(
        "[uploadToUserDrive] Errors array:",
    ...
    console.error(
      "[uploadToUserDrive] errorMessage for matching:",
      errorMessage,
    );
    console.error(
      "[uploadToUserDrive] responseStatus for matching:",
      responseStatus,
    );
    console.error(
      "[uploadToUserDrive] includes Bad Request?:",
    ...
    if (
      ...
      return {
        success: false,
        error:
          "Missing Google Drive permissions. Please sign out, then sign in again with Google to grant Drive access.",
      };
    }
    ...
    if (responseStatus === 400 || errorMessage.includes("Bad Request")) {
      ...
      const detail =
        specificError ||
        "The file may be invalid or too complex for Google Slides conversion.";
      console.error(
        "[uploadToUserDrive] Returning: 400 Bad Request with detail:",
        detail,
      );
      return {
        success: false,
...
> {
  ...
  console.log(
    "[uploadPptxToUserGoogleSlides] buffer size:",
    pptxBuffer.length,
    "bytes",
  );
  ...
  try {
    ...
    if (!tokenResult.success) {
      console.error(
        "[uploadPptxToUserGoogleSlides] FAIL: Token fetch failed:",
        tokenResult.error,
      );
      return tokenResult;
    }
    ...
    if (!uploadResult.success) {
      console.error(
        "[uploadPptxToUserGoogleSlides] FAIL: Upload failed:",
        uploadResult.error,
      );
      return uploadResult;
    }
    ...
    console.log(
      "[uploadPptxToUserGoogleSlides] url:",
    ...
    console.error(
      "[uploadPptxToUserGoogleSlides] UNEXPECTED ERROR (catch block)",
    );
    console.error(
      "[uploadPptxToUserGoogleSlides] Error type:",
      error?.constructor?.name,
    );
    console.error(
      "[uploadPptxToUserGoogleSlides] Error message:",
      error instanceof Error ? error.message : String(error),
    );
