...
export function getIM360Url({ gradeLevel, unitNumber, lessonNumber, section }: IM360UrlParams): string {
  ...
  const isAlgebra1 = gradeNum === '9' || gradeLevel.toLowerCase().includes('algebra');
  ...
  const sectionMatch = section.match(/Section ([A-Z])/i) || section.match(/^([A-Z])$/i);
  const sectionPath = sectionMatch ? `section-${sectionMatch[1].toLowerCase()}` : `section-${section.toLowerCase()}`;
