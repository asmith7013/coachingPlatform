...
export const BooleanFieldMappings = {
  ...
  modelingPlanningDone: {
    label:
      "Did you provide modeling or planning with teachers at this school today?",
    transformToForm: (value: boolean) => (value ? "Yes" : "No"),
    parseFromForm: (value: string) => value === "Yes",
  },
  walkthroughDone: {
  ...
  CoachingDone: {
    label:
      "Did you provide coaching, modeling, or planning with a group of teachers at this school today?",
    transformToForm: (value: boolean) => (value ? "Yes" : "No"),
    parseFromForm: (value: string) => value === "Yes",
  },
  adminMeet: {
...
export const NYCCoachMapping = {
  ...
  options: ["Yes, NYC Solves", "Yes, NYC Other", "No"],
};

/**
 * Transform schema boolean value to form string value
 */
export function transformBooleanForForm(
 ...
 */
export function parseBooleanFromForm(
  fieldName: keyof typeof BooleanFieldMappings,
  value: string,
): boolean {
  const mapping = BooleanFieldMappings[fieldName];
...
export function getSmartBooleanDefault(
  ...
  switch (fieldName) {
    ...
    case "microPLDone":
      ...
      return (
        !!(coachingLog.microPLTopic as string)?.trim() ||
    ...
    case "modelingPlanningDone":
      ...
      // Only infer from data if no explicit value AND there's actual content
      return (
        !!(coachingLog.modelTopic as string)?.trim() ||
