...
export function getImplementationOptions(experience: string) {
  return (
    IMPLEMENTATION_INDICATORS[
      experience as keyof typeof IMPLEMENTATION_INDICATORS
    ] || []
  );
}
...
export type ImplementationExperienceKey =
  keyof typeof IMPLEMENTATION_INDICATORS;
export type PrimaryStrategyCategoryKey = keyof typeof PRIMARY_STRATEGIES;
export type ImplementationIndicatorValue =
  (typeof IMPLEMENTATION_INDICATORS)[ImplementationExperienceKey][number];
export type PrimaryStrategyValue =
  (typeof PRIMARY_STRATEGIES)[PrimaryStrategyCategoryKey][number];
