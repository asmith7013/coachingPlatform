 ...
 */
function createTemplateFromSchema<T>(
...
function createBellScheduleTemplate(): Record<string, unknown> {
  ...
  // Validate template structure using transformer
  const validation = validateSafe(
    BellScheduleInputZodSchema.partial(),
    template,
  );
  if (!validation) {
...
function generateEnumDefinitions(templateFields: string[]): string {
  const relevantEnums = templateFields
    .filter(
      (field) => FIELD_TO_ENUM_MAP[field as keyof typeof FIELD_TO_ENUM_MAP],
    )
    .map((field) => {
      const enumMapping =
        FIELD_TO_ENUM_MAP[field as keyof typeof FIELD_TO_ENUM_MAP];
      const formattedEnum = formatEnumDefinition(
        enumMapping.name,
        enumMapping.enum,
      );
 ...
 */
export function createMasterSchedulePrompt(
  schoolStaff: Array<{ staffName: string; email: string }>,
): string {
  const staffList = schoolStaff
    .map((staff) => `- ${staff.staffName} (${staff.email})`)
    .join("\n");
