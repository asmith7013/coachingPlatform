...
export type EventColor = 'blue' | 'green' | 'purple' | 'yellow' | 'pink' | 'gray'
 ...
 */
export function getEventColorClasses(color: EventColor): typeof EVENT_COLOR_MAP[EventColor] {
 ...
 */
export function getEventContainerClasses(color: EventColor, selected?: boolean): string {
  const colorClasses = getEventColorClasses(color)
  const baseClasses = `${colorClasses.container} ${colorClasses.border}`
  return selected ? `${baseClasses} ring-2 ${colorClasses.selected}` : baseClasses
}
...
export const eventItem = tv({
  slots: {
    container: 'group absolute inset-1 flex flex-col overflow-y-auto rounded-lg p-2 text-xs/5 border transition-all duration-200',
