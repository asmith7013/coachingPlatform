...
const transformIPGCoreActionToReference = (
  coreAction: IPGCoreAction,
): IPGCoreActionReference => {
  const entityWithId = {
    ...coreAction,
...
const transformIPGCoreActionsToReferences = createArrayTransformer(
  transformIPGCoreActionToReference,
);
const transformIPGSubsectionsToReferences = createArrayTransformer(
  transformIPGSubsectionToReference,
);
const transformCoachingMovesToReferences = createArrayTransformer(
  transformCoachingMoveToReference,
);
...
export const getIPGSubsectionOptions = (
  coreActionNumber?: number,
): IPGSubsectionReference[] => {
  try {
    const coreActions = ipgData as IPGCoreAction[];
    ...
    if (coreActionNumber) {
      const coreAction = coreActions.find(
        (ca) => ca.coreAction === coreActionNumber,
      );
      if (coreAction) {
...
export const getCoachingMoveOptions = (
  category?: string,
): CoachingMoveReference[] => {
  try {
...
export const getCoachingMoveCategories = (): {
  value: string;
  label: string;
}[] => {
  try {
...
export const searchIPGOptions = (
  ...
  try {
    ...
    const coreActions = getIPGCoreActionOptions().filter(
      (ca) =>
        ca.label.toLowerCase().includes(lowerQuery) ||
        ca.title.toLowerCase().includes(lowerQuery),
    );

    const subsections = getIPGSubsectionOptions().filter(
      (sub) =>
...
export const searchCoachingMoves = (query: string): CoachingMoveReference[] => {
  try {
    ...
    return getCoachingMoveOptions().filter(
      (move) =>
