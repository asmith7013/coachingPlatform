...
export function validateStageCompleteness(
  ...
  try {
    if (
      stage === "needsAndFocus" &&
      planData.ipgCoreAction &&
      planData.ipgSubCategory &&
      planData.rationale
    ) {
    ...
    if (
      stage === "goal" &&
      planData.goalDescription.length > 0 &&
      planData.goalDescription.length > 0
    ) {
      const validated = validateSafe(CoachingActionPlanZodSchema, planData);
      if (!validated) {
...
export function calculatePlanProgress(
  ...
  const stageDetails = stages.map((stage) =>
    validateStageCompleteness(stage, planData),
  );
...
export const statusWorkflow: StatusWorkflow = {
  ...
  canTransition: (
    ...
    const transition = statusWorkflow.transitions.find(
    ...
    if (plan && transition.requires) {
      ...
      return transition.requires.every(
        (requiredStage) =>
  ...
  getNextStatuses: (
    ...
    return statusWorkflow.transitions
      .filter(
        (t) =>
          t.from === current &&
          statusWorkflow.canTransition(current, t.to, plan),
      )
      .map((t) => t.to);
  },
  ...
  validateTransition: (
    ...
    try {
      ...
      if (transition.requires) {
        const progress = calculatePlanProgress(plan);
        const missingStages = transition.requires.filter(
          (requiredStage) =>
            !progress.stageDetails.find((s) => s.stage === requiredStage)
              ?.isValid,
        );
      ...
      if (to === "completed") {
        ...
        if (!progress.canComplete) {
          errors.push(
            "Plan must be at least 80% complete to mark as completed",
          );
        }
      }
      ...
      if (to === "active" && from === "draft") {
        const hasBasicInfo =
          plan.title && plan.teachers?.length > 0 && plan.school;
        if (!hasBasicInfo) {
          errors.push(
            "Plan must have title, teachers, and school before activation",
          );
        }
      }
...
export function canBulkTransition(
  ...
  plans.forEach((plan) => {
    try {
      const validation = statusWorkflow.validateTransition(
