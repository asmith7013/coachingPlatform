 ...
 */
export function createBaseReference<
  T extends { _id: string | { toString(): string } },
>(entity: T, labelField: keyof T): BaseReference {
  return {
...
>(
  ...
  getAdditionalFields?: (
    entity: TEntity,
  ) => Partial<Omit<TReference, "_id" | "label" | "value">>,
  ...
  return function transformToReference(entity: TEntity): TReference {
    ...
    const reference: Record<string, unknown> = {
      _id: typeof entity._id === "string" ? entity._id : entity._id.toString(),
      value:
        typeof entity._id === "string" ? entity._id : entity._id.toString(),
 ...
 */
export function createArrayTransformer<T, R>(transformer: (entity: T) => R) {
