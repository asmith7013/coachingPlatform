...
export const CoachingActionPlanFieldsSchema = z.object({
  ...
  academicYear: z.string().default('2024-2025').describe("Academic year for this plan"),
  ...
  weeklyPlans: z.array(CapWeeklyPlanFieldsSchema).default([]).describe("Week-by-week coaching plans embedded in CAP"),
  outcomes: z.array(CapOutcomeFieldsSchema).default([]).describe("Expected outcomes and their embedded metrics"),
  evidence: z.array(CapEvidenceFieldsSchema).default([]).describe("Supporting evidence collected during coaching"),
  implementationRecords: z.array(CapImplementationRecordFieldsSchema).default([]).describe("Records of actual coaching sessions"),
  
  // Keep visit references (visits remain separate due to scheduling complexity)
  relatedVisitsIds: z.array(z.string()).default([]).describe("Array of Visit IDs related to this plan"),
  relatedCyclesIds: z.array(z.string()).default([]).describe("Array of Cycle IDs related to this plan"),
});
