...
export const StudentActivityFieldsSchema = z.object({
  ...
  unitNumber: z.number().optional().describe("Unit number for easier filtering"),
  ...
  loggedAt: z.string().optional().describe("When activity was logged (ISO timestamp)"),
});

/**
 * Full student activity schema with base document fields
 */
export const StudentActivityZodSchema = BaseDocumentSchema.merge(StudentActivityFieldsSchema);

/**
 * Input schema for creating student activities
 */
export const StudentActivityInputZodSchema = toInputSchema(StudentActivityZodSchema);
...
export type StudentActivityEventInput = z.infer<typeof StudentActivityInputZodSchema>;
