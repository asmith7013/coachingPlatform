...
export type WorkedExampleRequestStatus = z.infer<
  typeof WorkedExampleRequestStatusZod
>;
...
export const WorkedExampleRequestFieldsSchema = z.object({
  ...
  scopeSequenceTag: ScopeSequenceTagZod.describe(
    "Curriculum tag (Grade 6, Grade 7, Grade 8, Algebra 1)",
  ),
  ...
  lessonNumber: z
    .number()
    .int()
    .describe("Lesson number (can be 0 or negative for ramp-ups)"),
  ...
  section: LessonSectionZod.optional().describe(
    "Lesson section within the unit (A, B, C, etc.)",
  ),
  ...
  roadmapSkills: z
    .array(z.string())
    .default([])
    .describe("All roadmap skill numbers from the lesson"),
  targetSkills: z
    .array(z.string())
    .default([])
    .describe("Target skill numbers from the lesson"),

  // =====================================
  // TEACHER INPUT: WHAT STUDENT IS STRUGGLING WITH
  // =====================================
  strugglingSkillNumbers: z
    .array(z.string())
    .min(1)
    .describe("Which skills teacher thinks student struggles with"),
  strugglingDescription: z
    .string()
    .min(10)
    .describe("Free text: describe the misconception/struggle"),
  ...
  learningGoals: z
    .array(z.string())
    .default([])
    .describe("Student-facing learning objectives"),
  ...
  sourceImageUrl: z
    .string()
    .url()
    .describe("Vercel Blob URL of uploaded practice problem image"),
  sourceImageFilename: z
    .string()
    .describe("Original filename of the uploaded image"),
  ...
  additionalNotes: z
    .string()
    .optional()
    .describe("Additional context or notes from teacher"),
  ...
  requestedByEmail: z
    ...
    .email()
    .optional()
    .describe("Email for notifications"),
  completedWorkedExampleId: z
    .string()
    .optional()
    .describe("MongoDB ObjectId of created worked example deck when completed"),
});

// Full schema with base document fields
export const WorkedExampleRequestZodSchema = BaseDocumentSchema.merge(
  WorkedExampleRequestFieldsSchema,
);

// Input schema for creation (excludes _id, id, createdAt, updatedAt)
export const WorkedExampleRequestInputZodSchema = toInputSchema(
  WorkedExampleRequestZodSchema,
);
...
export type WorkedExampleRequest = z.infer<
  typeof WorkedExampleRequestZodSchema
>;
export type WorkedExampleRequestInput = z.infer<
  typeof WorkedExampleRequestInputZodSchema
>;
...
export type WorkedExampleRequestQuery = z.infer<
  typeof WorkedExampleRequestQuerySchema
>;
