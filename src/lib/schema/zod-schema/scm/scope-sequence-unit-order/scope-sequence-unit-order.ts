...
export const UnitOrderItemZod = z.object({
  ...
  unitNumber: z
    ...
    .int()
    .positive()
    .describe("The unitNumber in scope-and-sequence collection"),
  unitName: z.string().describe("Full unit name for display"),
  grade: GradeZod.describe(
    "Grade field from scope-and-sequence (for Algebra 1: '8' or 'Algebra 1')",
  ),
});
...
export const ScopeSequenceUnitOrderFieldsSchema = z.object({
  scopeSequenceTag: ScopeSequenceTagZod.describe(
    "Curriculum identifier (Grade 6, Grade 7, Grade 8, or Algebra 1)",
  ),
  units: z
    .array(UnitOrderItemZod)
    .describe("Ordered array of units for this curriculum"),
});

// Full Schema with base document fields
export const ScopeSequenceUnitOrderZodSchema = BaseDocumentSchema.merge(
  ScopeSequenceUnitOrderFieldsSchema,
);

// Input Schema (for creation)
export const ScopeSequenceUnitOrderInputZodSchema = toInputSchema(
...
export type ScopeSequenceUnitOrder = z.infer<
  typeof ScopeSequenceUnitOrderZodSchema
>;
export type ScopeSequenceUnitOrderInput = z.infer<
  typeof ScopeSequenceUnitOrderInputZodSchema
>;
