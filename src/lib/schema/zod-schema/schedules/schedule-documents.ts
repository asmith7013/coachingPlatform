...
const BaseScheduleFields = BaseDocumentSchema.extend({
  ...
  dayIndices: z.array(DayIndexZod).describe("Days when this schedule applies: [0,1,2,3,4] for M-F, [4] for Friday early release"),
});
...
export const BellScheduleZodSchema = BaseScheduleFields.extend({
  ...
  name: z.string().describe("Schedule name: 'Regular Day', 'Mon/Wed Block', 'A Day', etc."),
  bellScheduleType: BellScheduleTypeZod.describe("Schedule pattern type"),
  timeBlocks: z.array(BellScheduleBlockSchema).describe("Array of bell schedule time blocks"),
});
...
export const TeacherScheduleZodSchema = BaseScheduleFields.extend({
  ...
  timeBlocks: z.array(TeacherScheduleBlockSchema).describe("Period assignments for the specified days"),
});
...
export const VisitScheduleZodSchema = BaseScheduleFields.extend({
  ...
  timeBlocks: z.array(VisitScheduleBlockSchema).describe("Flat array of visit time blocks"),
});
...
export type TeacherScheduleReference = z.infer<typeof TeacherScheduleReferenceSchema>;
export type VisitScheduleReference = z.infer<typeof VisitScheduleReferenceSchema>;
...
export function bellScheduleToReference(schedule: BellSchedule): BellScheduleReference {
  return {
    _id: schedule._id,
    value: schedule._id,
...
export function teacherScheduleToReference(schedule: TeacherSchedule): TeacherScheduleReference {
  return {
    _id: schedule._id,
    value: schedule._id,
...
export function visitScheduleToReference(schedule: VisitSchedule): VisitScheduleReference {
  return {
    _id: schedule._id,
    value: schedule._id,
    ...
    dateFormatted: schedule.date ? new Date(schedule.date).toLocaleDateString() : undefined,
  };
}

export function schedulesToReferences(schedules: BellSchedule[] | TeacherSchedule[] | VisitSchedule[]): ScheduleReference[] {
