...
export const BellScheduleFieldsSchema = z.object({
  ...
  dayIndices: z.array(DayIndexZod).describe("Days when this bell schedule applies: [0,1,2,3,4] for M-F, [4] for Friday early release"),
...
export const TeacherScheduleFieldsSchema = z.object({
  ...
  dayIndices: z.array(DayIndexZod).describe("Days when this entire schedule applies: [0,1,2,3,4] for M-F, [0,2,4] for M/W/F"),
  
  // Clean assignments array (applies to all days in dayIndices)
  assignments: z.array(PeriodFieldsSchema).describe("Period assignments for the specified days")
});
...
export const EventFieldsSchema = z.object({
  ...
  // Timing information
  timeSlot: TimeBlockFieldsSchema.optional().describe("More specific time window from bell schedule"),
...
export const VisitScheduleFieldsSchema = z.object({
  ...
  events: z.array(EventFieldsSchema).describe("Flat array of all events - use periodNumber + orderIndex for sequencing")
});
