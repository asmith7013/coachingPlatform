...
export const CapMetricFieldsSchema = z.object({
  ...
  type: z
    .string()
    .default("")
    .describe("Type of metric (quantitative, qualitative, etc.)"),
  description: z.string().default("").describe("What will be measured"),
  collectionMethod: MetricCollectionMethodZod.default(
    MetricCollectionMethodZod.options[0],
  ).describe("How the metric will be collected"),
  baselineValue: z
    .string()
    .optional()
    .default("")
    .describe("Starting value for this metric"),
  targetValue: z.string().default("").describe("Goal or target value"),
  currentValue: z
    .string()
    .optional()
    .default("")
    .describe("Current measured value"),
  notes: z
    .string()
    .optional()
    .default("")
    .describe("Additional notes about this metric"),
});

// Keep standalone document schemas for backward compatibility and potential future use
export const CapMetricZodSchema = BaseDocumentSchema.merge(
  CapMetricFieldsSchema,
);
...
export function createCapMetricDefaults(
  overrides: Partial<CapMetricInput> = {},
): CapMetricInput {
  return {
