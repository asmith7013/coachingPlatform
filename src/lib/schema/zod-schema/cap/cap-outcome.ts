...
export const CapOutcomeFieldsSchema = z.object({
  ...
  type: z
    .string()
    .default("")
    .describe("Type of outcome (teacher-facing or student-facing)"),
  description: z.string().default("").describe("Expected outcome or change"),
  achieved: z
    .boolean()
    .optional()
    .default(false)
    .describe("Was this outcome achieved?"),
  sortOrder: z.number().default(0).describe("For maintaining order"),
  metrics: z
    .array(CapMetricFieldsSchema)
    .default([])
    .describe("Metrics that measure this outcome"),
  evidence: z
    .array(CapEvidenceFieldsSchema)
    .optional()
    .default([])
    .describe("Evidence supporting this outcome"),
});

// Keep standalone document schemas for backward compatibility and potential future use
export const CapOutcomeZodSchema = BaseDocumentSchema.merge(
  CapOutcomeFieldsSchema.extend({
...
export function createCapOutcomeDefaults(
  overrides: Partial<CapOutcomeInput> = {},
): CapOutcomeInput {
  return {
