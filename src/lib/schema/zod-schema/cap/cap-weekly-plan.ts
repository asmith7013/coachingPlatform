...
import { VisitStatusZod, CoachingCycleNumberZod, VisitNumberZod } from "@enums";
...
export const CapWeeklyPlanFieldsSchema = z.object({
  ...
  lookFor: z
    .string()
    .default("")
    .describe("Specific things to observe or look for"),
  coachAction: z.string().default("").describe("Actions the coach will take"),
  teacherAction: z
    .string()
    .default("")
    .describe("Actions expected from the teacher"),
  ...
  status: VisitStatusZod.default("planned").describe(
  ...
  expectedMetrics: z
    ...
    .default([])
    .describe(
      "Outcome IDs containing metrics expected to be measured this week",
    ),
});

// Keep standalone document schemas for backward compatibility and potential future use
export const CapWeeklyPlanZodSchema = BaseDocumentSchema.merge(
  CapWeeklyPlanFieldsSchema,
);
export const CapWeeklyPlanInputZodSchema = toInputSchema(
...
export function createCapWeeklyPlanDefaults(
  overrides: Partial<CapWeeklyPlanInput> = {},
): CapWeeklyPlanInput {
  return {
