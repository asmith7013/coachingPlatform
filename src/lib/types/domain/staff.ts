...
export type {
  ...
  export function determineStaffType(staff: Record<string, unknown>): StaffType {
    if ('adminLevel' in staff || 'rolesTL' in staff || 'assignedDistricts' in staff) {
      return 'tl';
    }
    
    if ('gradeLevelsSupported' in staff || 'rolesNYCPS' in staff || 'subjects' in staff || 'specialGroups' in staff) {
      return 'nycps';
    }
