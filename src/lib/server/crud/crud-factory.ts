...
export function createCrudActions<T extends BaseDocument, TInput = Partial<T>>(
  ...
  const inputValidator = inputSchema
    ? createValidator(inputSchema, name)
    : null;
  const partialValidator = inputSchema
    ? createValidator(inputSchema, name, { partial: true })
    : null;
  ...
  return {
    ...
    async create(data: TInput): Promise<EntityResponse<T>> {
      try {
        ...
        if (inputValidator) {
          const validation = inputValidator.validateSingle(
            JSON.stringify(data),
          );
    ...
    async update(
      ...
      try {
        ...
        if (partialValidator && Object.keys(data).length > 0) {
          ...
          const validation = partialValidator.validateSingle(
            JSON.stringify(data),
          );
          if (!validation.success) {
            const validationError = new Error(validation.error);
