...
import { buildQueryParams } from '@data-utilities/pagination/pagination-utils';
...
export function useQueryState(initialParams: Partial<QueryParams> = {}) {
  ...
  const updateQueryParams = useCallback((newParams: Partial<QueryParams>) => {
    setQueryParams(prev => buildQueryParams({
      ...prev,
      ...newParams
    }));
  }, []);
  ...
  const changeSorting = useCallback((newSortBy: string, newSortOrder?: 'asc' | 'desc') => {
    updateQueryParams({ 
      sortBy: newSortBy,
      sortOrder: newSortOrder || (newSortBy === sortBy && sortOrder === 'asc' ? 'desc' : 'asc')
    });
