...
export const optimisticUpdateHelpers = {
  ...
  >(
    ...
    queryClient.setQueryData([entityType, "list"], (old: unknown) => {
  ...
  optimisticUpdate: <T extends BaseDocument, D extends Record<string, unknown>>(
    ...
    queryClient.setQueryData([entityType, "detail", id], (old: unknown) => {
      if (!old) return old;
  ...
  handleMutationError: <T extends BaseDocument>(
    ...
    if (context?.previousData) {
      queryClient.setQueryData([entityType, "list"], context.previousData);
    }
  },
};
