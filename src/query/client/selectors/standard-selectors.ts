      ...
      try {
        ...
        return {
          ...
          formattedCreatedAt: item.createdAt
            ? formatMediumDate(toDateString(new Date(item.createdAt)))
            : undefined,
          formattedUpdatedAt: item.updatedAt
            ? formatMediumDate(toDateString(new Date(item.updatedAt)))
            : undefined,
        };
      } catch (error) {
...
function getNYCPSStaffSelectors() {
  try {
    return createSpecializedSelectors(
      registerSelector(
        "nycps-staff",
        NYCPSStaffZodSchema as ZodType<NYCPSStaff>,
      ),
      "nycps-staff",
    );
  } catch (error) {
...
function getTeachingLabStaffSelectors() {
  try {
    return createSpecializedSelectors(
      registerSelector(
        "teaching-lab-staff",
        TeachingLabStaffZodSchema as ZodType<TeachingLabStaff>,
      ),
      "teaching-lab-staff",
    );
  } catch (error) {
...
function getCoachingLogSelectors() {
  try {
    return createSpecializedSelectors(
      registerSelector(
        "coaching-logs",
        CoachingLogZodSchema as ZodType<CoachingLog>,
      ),
      "coaching-logs",
    );
  } catch (error) {
...
export const standardSelectors = {
  get schools() {
    return getSchoolSelectors();
  },
  get staff() {
    return getStaffSelectors();
  },
  get nycpsStaff() {
    return getNYCPSStaffSelectors();
  },
  get teachingLabStaff() {
    return getTeachingLabStaffSelectors();
  },
  get cycles() {
    return getCycleSelectors();
  },
  get visits() {
    return getVisitSelectors();
  },
  get coachingLogs() {
    return getCoachingLogSelectors();
  },
  get lookFors() {
    return getLookForSelectors();
  },
  get rubrics() {
    return getRubricSelectors();
  },
};
...
export const teachingLabStaffSelector = () =>
  getTeachingLabStaffSelectors()?.base;
